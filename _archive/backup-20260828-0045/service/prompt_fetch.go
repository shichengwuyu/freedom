...
const (
	gptImage2RawBase             = "https://gcore.jsdelivr.net/gh/tigerowo/awesome-gpt-image-2-prompts@main"
	awesomeGptImageRawBase       = "https://gcore.jsdelivr.net/gh/ZeroLu/awesome-gpt-image@main"
	awesomeGpt4oImagePromptsBase = "https://gcore.jsdelivr.net/gh/ImgEdify/Awesome-GPT4o-Image-Prompts@main"
	youMindGptImage2RawBase      = "https://gcore.jsdelivr.net/gh/YouMind-OpenLab/awesome-gpt-image-2@main"
	youMindNanoBananaProRawBase  = "https://gcore.jsdelivr.net/gh/YouMind-OpenLab/awesome-nano-banana-pro-prompts@main"
	youMindSeedance2RawBase      = "https://gcore.jsdelivr.net/gh/YouMind-OpenLab/awesome-seedance-2-prompts@main"
	youMindGrokImagineRawBase    = "https://gcore.jsdelivr.net/gh/YouMind-OpenLab/awesome-grok-imagine-prompts@main"
	xianyuAwesomeGptImage2RawBase = "https://gcore.jsdelivr.net/gh/xianyu110/awesome-gptimage2@main"
	davidWuGptImage2RawBase      = "https://gcore.jsdelivr.net/gh/davidwuw0811-boop/awesome-gpt-image2-prompts@main"

	imageCategoryMaxItems = 100
	videoCategoryMaxItems = 100
)
...
func buildPromptCategory(category string) ([]model.Prompt, error) {
	switch category {
	case "image":
		// 图片分类：聚合 7 个 GitHub 仓库，统一入库，最多保留 imageCategoryMaxItems 条
		return aggregatePrompts([]func() ([]model.Prompt, error){
			buildGptImage2Prompts,
			buildXianyuAwesomeGptImage2Prompts,
			buildDavidWuGptImage2Prompts,
			buildYouMindGptImage2Prompts,
			buildYouMindNanoBananaProPrompts,
			buildAwesomeGptImagePrompts,
			buildAwesomeGpt4oImagePrompts,
		}, imageCategoryMaxItems)
	case "video":
		// 视频分类：仅聚合 YouMind 的 Seedance 2.0 视频提示词仓库
		return aggregatePrompts([]func() ([]model.Prompt, error){
			buildYouMindSeedance2Prompts,
		}, videoCategoryMaxItems)
	}
	return nil, nil
}

// aggregatePrompts 聚合多个提示词构建器，最多保留 maxItems 条，单个构建器失败时跳过。
func aggregatePrompts(builders []func() ([]model.Prompt, error), maxItems int) ([]model.Prompt, error) {
	merged := []model.Prompt{}
	for i, build := range builders {
		items, err := build()
		if err != nil {
			log.Printf("build prompt source[%d] failed err=%v", i, err)
			continue
		}
...
const promptFetchMaxBytes = 16 << 20 // 16MB

func fetchText(baseURL, file string) (string, error) {
	request, err := http.NewRequest(http.MethodGet, baseURL+"/"+file, nil)
	if err != nil {
		return "", err
	}
	...
	data, err := io.ReadAll(io.LimitReader(response.Body, promptFetchMaxBytes+1))
	if err != nil {
		return "", err
	}
	if len(data) > promptFetchMaxBytes {
		return "", errors.New(file + " 超过大小限制")
	}
...
func buildYouMindSeedance2Prompts() ([]model.Prompt, error) {
	return buildYouMindPrompts(youMindSeedance2RawBase, "youmind-seedance-2", "seedance-2")
}

func buildYouMindGrokImaginePrompts() ([]model.Prompt, error) {
	return buildYouMindPrompts(youMindGrokImagineRawBase, "youmind-grok-imagine", "grok-imagine")
}
...
func buildYouMindPrompts(baseURL, idPrefix, modelTag string) ([]model.Prompt, error) {
	...
	for _, block := range splitBeforeHeading(markdown, "### ") {
		...
		// 提取视频直链（mp4），格式如 <a href="https://github.com/.../videos/1402.mp4">
		videoURL := firstMatch(block, `<a href="([^"]+\.mp4)"`)
		// 提取 youmind.com 外部视频链接，格式如 [🎬 查看视频 →](https://youmind.com/...)
		externalURL := firstMatch(block, `\[🎬\s*查看视频[^\]]*\]\((https://youmind\.com/[^)]+)\)`)
		if externalURL == "" {
			externalURL = firstMatch(block, `\[👉\s*立即体验[^\]]*\]\((https://youmind\.com/[^)]+)\)`)
		}
		items = append(items, model.Prompt{ID: idPrefix + "-" + leftPad(len(items)+1), Title: title, CoverURL: cover, VideoURL: videoURL, ExternalURL: externalURL, Prompt: prompt, Tags: youMindTags(title, modelTag), Preview: markdownPreview(images)})
	}
	return items, nil
}
