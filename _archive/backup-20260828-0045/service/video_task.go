...
func CreateVideoTask(input VideoTaskCreateInput) (model.VideoTask, error) {
	...
	task := model.VideoTask{
		// PR-8：主键始终服务端生成，禁止把客户端可控的 ClientTaskID / UpstreamTaskID
		// 当作 id，否则不同用户传相同上游 ID 会互相覆盖。
		// UpstreamTaskID / UpstreamVideoID 字段仍保留作轮询关联键。
		ID:              newVideoTaskID(),
...
// PR-8: 服务端生成的主键，参见 CreateVideoTask 注释。
func newVideoTaskID() string {
	return "video-task-" + uuid.NewString()
}
