...
func CurrentUserConfig(ctx context.Context) (UserConfigPayload, error) {
	...
	if strings.TrimSpace(config.StorageProvider) != "" {
		...
		// PR-10：响应前清空明文凭据。syncFlags 之前只控制整节点是否返回，但 secret 字段本身
		// 仍被原样透出 —— 任何登录用户调一次 GET 就能拿到完整 AK/SK / WebDAV 密码。
		// 写回时由 SaveCurrentUserStorageProvider 合并保留原值（前端编辑非密钥字段时不会丢密钥）。
