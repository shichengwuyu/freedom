...
// 自动定价定时拉取调度器（2026-08-21 引入；2026-08-27 改为 15 分钟一次）。
//
// 每 15 分钟从 PricingURL 拉取一次上游模型定价（人民币元），按「视频 +50%、图片 +20%」的加价率
...
const (
	// pricingCron 自动定价拉取频率：每 15 分钟一次（上游定价会实时变动，缩短间隔让新增/改价模型更快生效）
	pricingCron        = "*/15 * * * *"
...
// isAllDigits 判断字符串是否全部由 ASCII 数字组成（空串返回 false）。
// 2026-08-27：从 service/vendor.go 迁出（vendor.go 已被整体删除）。
