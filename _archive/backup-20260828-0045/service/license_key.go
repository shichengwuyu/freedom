...
// PR-9：走 repository.ListUserBalanceLogs，user_id 硬绑定 + keyword 限非用户标识列，
// 不再用 ListBalanceLogs 那种跨列模糊查询，避免"用户 A 的 ID 出现在 B 的 remark/related_id
// 里时被 B 搜到"的跨用户隐私泄漏。
func ListMyBalanceLogs(userID string, q model.Query) (model.BalanceLogList, error) {
	logs, total, err := repository.ListUserBalanceLogs(userID, model.Query{
		Keyword:  q.Keyword,
		Page:     q.Page,
		PageSize: q.PageSize,
	})
