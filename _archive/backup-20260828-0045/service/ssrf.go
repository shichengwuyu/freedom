...
var directStorageHTTPClient = newDirectStorageHTTPClient()

// SafeProxyHTTPClient 面向第三方/外部不可信 URL 的请求，启用 SSRF 内网屏蔽。
func SafeProxyHTTPClient() *http.Client {
	return safeProxyHTTPClient
}

// DirectStorageHTTPClient 面向管理员或用户自行配置的对象存储 provider（S3/WebDAV），
// 这类 Endpoint 由可信赖的一方写入（允许内网/loopback，例如 docker-compose 内的 minio:9000）。
func DirectStorageHTTPClient() *http.Client {
	return directStorageHTTPClient
}
...
// newDirectStorageHTTPClient 不使用 SSRF 内网屏蔽，仅用于可信存储 provider。
func newDirectStorageHTTPClient() *http.Client {
	transport := http.DefaultTransport.(*http.Transport).Clone()
	transport.Proxy = nil
	return &http.Client{
		Transport: transport,
