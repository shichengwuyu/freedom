...
// errClientWriteFailed 哨兵错误：流式响应复制过程中 client 断开 / w.Write 失败。
// copyAIResponse 在响应已写头部之后若仍发生写失败，应取消（而非结算）余额占用，
// 避免给没收到完整响应的用户扣费。
var errClientWriteFailed = errors.New("ai proxy: client write failed")
...
func proxyAIGetRequest(w http.ResponseWriter, r *http.Request, path string) {
	...
	// PR-6：把 r.Context() 透传到上游请求。client 断开时 ctx 取消 → http.Client.Do 立即中断，
	// 配合下面的 copyAIResponseBody 写错分支取消 hold，避免给已断开的用户继续扣费。
...
func copyAIResponse(w http.ResponseWriter, request *http.Request, channel model.ModelChannel, logContext aiLogContext, onFailure func()) {
	...
	written, copyErr := copyAIResponseBody(w, response.Body)
	// 流式写失败：client 断开 / 网络断 → 取消 hold，避免给没收到响应的用户扣费。
	// 已 WriteHeader 之后无法再改 status code，只能把错误写进 log。
	if copyErr != nil {
		if errors.Is(copyErr, errClientWriteFailed) {
			log.Printf("AI proxy client write failed mid-stream: model=%s written=%d err=%v", logContext.Model, written, copyErr)
			holdCancel(logContext.HoldID, "client write failed")
			if onFailure != nil {
				onFailure()
			}
			saveAIProxyLog(logContext, response.StatusCode, "", copyErr.Error())
			return
		}
		// 上游 Read 出错（不是 client 写错）：同样不应结算，让用户拿到退款。
		log.Printf("AI proxy upstream read failed mid-stream: model=%s written=%d err=%v", logContext.Model, written, copyErr)
		holdCancel(logContext.HoldID, "upstream read failed")
		if onFailure != nil {
			onFailure()
		}
		saveAIProxyLog(logContext, response.StatusCode, "", copyErr.Error())
	...
	// 注：responseBody 摘要已不再由 copyAIResponseBody 返回，saveAIProxyLog 在此落空 body 即可，
	// 前端拿响应内容（图片/文本）即相当于拿到了原始 payload，无需再 buffer 一份。
...
// copyAIResponseBody 把上游 body 复制到 w，按 32KB chunk 写并尽量 Flush。
// 返回 (写入字节数, 错误)：
//   - 正常 EOF 时 err=nil；
//   - 上游 Read 出错时 err=该错误（不视作 client 失败，hold 走 settle 逻辑由 copyAIResponse 决定）；
//   - w.Write 失败时返回 errClientWriteFailed（调用方应取消 hold + 改 5xx 响应）。
// 返回的 logBuffer 同时作为 ai_proxy_log 的 responseBody 摘要（最大 64KB）。
func copyAIResponseBody(w http.ResponseWriter, body io.Reader) (int64, error) {
	...
	for {
		...
		if n > 0 {
			if _, writeErr := w.Write(buffer[:n]); writeErr != nil {
				return written, fmt.Errorf("%w: %v", errClientWriteFailed, writeErr)
			}
			written += int64(n)
