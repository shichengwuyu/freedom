...
func UploadReferenceMedia(w http.ResponseWriter, r *http.Request) {
	...
	// 读取路由在 router.go 中挂在 /api/v1 分组下（带 UserAuth），因此返回的 URL 也必须带 /v1，
	// 否则浏览器按 /api/media/references/<id> 加载时会直接命中 NoRoute 404。
	// 若已配置 PUBLIC_BASE_URL 则返回完整公网 URL；未配置时返回站内相对路径，满足头像上传等仅需站内访问的场景
	var fileURL string
	if publicBaseURL != "" {
		fileURL = fmt.Sprintf("%s/api/v1/media/references/%s", publicBaseURL, id)
	} else {
		fileURL = fmt.Sprintf("/api/v1/media/references/%s", id)
	}
