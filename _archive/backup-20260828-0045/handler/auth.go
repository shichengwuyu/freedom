...
// setAuthCookie 把 JWT 写入 httpOnly + Secure cookie，前端通过 withCredentials 自动带上，
// 不再通过 localStorage 或 Authorization 头长期持有 token，避免 XSS 窃取与登出失效。
...
// clearAuthCookie 清除 httpOnly auth cookie。Log 与 set 对称。
...
// Logout 撤销当前会话：清掉 auth cookie，让后续请求在中间件里被识别为未登录。
// 调用方负责同步清掉客户端 store（避免 XSS 拿到的旧 store 还在带 user）。
// 该接口挂在 /api/v1（需登录），未登录调用由 UserAuth 中间件直接返回 401。
