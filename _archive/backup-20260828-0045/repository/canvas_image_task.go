...
// SaveCanvasImageTask 新建任务（PR-8：主键已服务端生成，这里用 INSERT 语义；where 仍带 user_id
// 是为了防御性兜底：即使将来有调用方误把 ClientTaskID 之类的客户端字段当成 id 传进来，
// 也不会跨用户覆盖别人的行）。Update 路径请用 UpdateCanvasImageTask（已带 owner where）。
