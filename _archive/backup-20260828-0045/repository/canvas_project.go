...
func ListUserCanvasProjects(userID string) ([]model.CanvasProject, error) {
	...
	// Limit 兜底（model.MaxPageSize=500）：避免用户累计几千个 canvas project 时一次拉全部压垮前端。
	// 后续如需分页再加 Page/PageSize 参数 + Normalize()。
	var projects []model.CanvasProject
	err = db.Where(
		"user_id = ? AND deleted_at = ''",
		strings.TrimSpace(userID),
	).Order("updated_at DESC").Limit(model.MaxPageSize).Find(&projects).Error
	return projects, err
}
