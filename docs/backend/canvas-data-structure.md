...
- 画布项目 JSON：`localForage`，数据库名 `freedom`，storeName `app_state`，key 为 `freedom:canvas_store`。
- 我的素材 JSON：`localForage`，数据库名 `freedom`，storeName `app_state`，key 为 `freedom:asset_store`。
- 图片 Blob：单独存到 `localForage` 实例，数据库名 `freedom`，storeName `image_files`。
- 视频等媒体 Blob：单独存到 `localForage` 实例，数据库名 `freedom`，storeName `media_files`。
