...
import { normalizeImageSizeForModel } from "@/lib/image-model-capabilities";
import { isMimoChannel, mimoModels } from "@/lib/mimo-tts";
import { imageToDataUrl, resolveImageUrl, resolveReferenceDataUrls } from "@/services/image-storage";
import { activeChannelKind, buildApiUrl, channelIdForActiveModel, directAIProviderForConfig, localChannelForActiveModel, type AiConfig } from "@/stores/use-config-store";
...
export type CanvasImageTaskOptions = { nodeId?: string; source?: "canvas" | "image-workbench" | "workflow" | "novel"; sourceId?: string; clientTaskId?: string };
...
function createImageRequestParams(config: AiConfig): ImageRequestParams {
    ...
    // 请求层兜底：存储的 size 可能因模型切换/旧节点而不合法（如 Gemini 节点的 "21:9"），
    // 先按当前模型归一化，保证发出去的永远是合法比例/尺寸。
    // Gemini 图片模型（gemini-3-pro-image-preview / nano-banana / imagen 等）上游不接受缺失 size
    // 或 "auto"，会返回 "unsupported image size; use a supported aspect ratio like 3:4"，
    // 因此当用户未显式选择尺寸时，补一个该模型白名单内的默认比例 "1:1"，再由 resolveSize 折算成像素。
    let size = config.size || "";
    if ((!size || size === "auto") && isGeminiImageModel(model)) {
        size = "1:1";
    }
    return {
        n: normalizeBoundedInteger(config.count, 1, 1, 15),
        quality,
        size: resolveRequestSize(quality, normalizeImageSizeForModel(model, size)),
...
function applyImageGenerationParams(body: Record<string, unknown>, config: AiConfig, params: ImageRequestParams, operation: "generation" | "edit" = "generation") {
    ...
    // Gemini 图片模型（如 rolldek 的 gemini-3-*-image-preview）不认 OpenAI 的 quality 字段，
    // 带上会让上游返回 400 / 挂起，故跳过。
    if (params.quality && !config.codexCli && !isGeminiImageModel(model)) body.quality = params.quality;
}
...
function usesAccountProxy(config: AiConfig) {
    ...
    const kind = activeChannelKind(config);
    return kind === "remote" || (kind === "local" && Boolean(token));
}
...
export function aiHeaders(config: AiConfig, contentType?: string) {
    const token = useUserStore.getState().token;
    const kind = activeChannelKind(config);
    if (kind === "remote" && !token) throw new Error("请先登录后再使用云端渠道");
    if (kind === "remote") {
        return {
...
// aiHeadersWithRequestId 在 aiHeaders 基础上叠加 X-Request-Id 头（2026-08-17 引入）。
// 后端 handler/ai.go 的 readRequestID 会用这个 header 当 ConsumeUserBalanceWithHold 的 requestID，
// 让前端 clientTaskId 透传成"幂等扣款键"，网络重试同一请求不会双扣。
export function aiHeadersWithRequestId(config: AiConfig, contentType?: string, clientTaskId?: string) {
    return {
        ...aiHeaders(config, contentType),
        ...(clientTaskId ? { "X-Request-Id": clientTaskId } : {}),
    };
}
...
async function writeLocalAICallLog(config: AiConfig, endpoint: string, startedAt: number, status: number, timeoutSeconds: number, requestBody: string, responseBody: string, error: string) {
    if (activeChannelKind(config) !== "local" || usesAccountProxy(config)) return;
    const token = useUserStore.getState().token;
    if (!token) return;
    const channel = localChannelForActiveModel(config);
    await fetch("/api/v1/ai-logs", {
        ...
        body: JSON.stringify({
            ...
            channelName: channel?.name || "自定义渠道",
            status,
...
async function requestImageGenerationSingle(config: AiConfig & { seedIndex?: number; seedCount?: number }, prompt: string, params: ImageRequestParams, clientTaskId?: string): Promise<GeneratedImage[]> {
                ...
                requestWithTransientRetry(() =>
                    withTimeout(params.timeoutSeconds, (signal) =>
                        fetch(aiApiUrl(config, "/images/generations"), {
                            method: "POST",
                            headers: aiHeadersWithRequestId(config, "application/json", clientTaskId),
...
async function requestGrokImageEditSingle(config: AiConfig, prompt: string, references: ReferenceImage[], params: ImageRequestParams, clientTaskId?: string): Promise<GeneratedImage[]> {
    const mime = IMAGE_MIME;
...
async function requestImageEditSingle(config: AiConfig, prompt: string, references: ReferenceImage[], params: ImageRequestParams, clientTaskId?: string): Promise<GeneratedImage[]> {
    if (isGrokImageModel(config.model)) return requestGrokImageEditSingle(config, prompt, references, params, clientTaskId);
        ...
        () =>
            requestWithTransientRetry(() =>
                withTimeout(params.timeoutSeconds, (signal) =>
                    fetch(aiApiUrl(config, "/images/edits"), {
                        method: "POST",
                        headers: aiHeadersWithRequestId(config, clientTaskId),
                        body: formData,
                        signal,
                    }),
                ),
...
async function requestResponsesSingle(config: AiConfig, prompt: string, inputImageDataUrls: string[], params: ImageRequestParams, clientTaskId?: string): Promise<GeneratedImage[]> {
    const mime = IMAGE_MIME;
    const body: Record<string, unknown> = {
        model: config.model,
...
async function requestImages(config: AiConfig & { seedIndex?: number; seedCount?: number }, prompt: string, references: ReferenceImage[], clientTaskId?: string): Promise<GeneratedImage[]> {
    const params = createImageRequestParams(config);
    const inputImageDataUrls = references.length ? await resolveReferenceDataUrls(references) : [];
    // Agnes 模型在 /responses 端点上游会抛 float('') 500，统一回退到 /images/generations 简单端点（applyAgnesImageSize 已处理 size/ratio 映射）
    // 供应商模式同理：走 /images/generations，由后端 vendor dispatch 接管
    // OpenAI 标准图片模型（gpt-image-2、dall-e-3）走 /images/generations，避免在 OpenAI 兼容代理（rolldek 等）上 /v1/responses 404。
    const useSimpleEndpoint = (config.activeVendorType && config.activeVendorType !== "official") || isAgnesImageModel(config.model) || isOpenAIImageGenerationModel(config.model) || isGeminiImageModel(config.model);

    const useConcurrentSingleRequests = true; // n>1 时并发单发更稳定
    if (params.n > 1 && useConcurrentSingleRequests) {
        const results = await Promise.allSettled(Array.from({ length: params.n }, () => requestImages({ ...config, count: "1" }, prompt, references, clientTaskId)));
    ...
    if (useSimpleEndpoint) {
        return requestImageGenerationSingle(config, prompt, params, clientTaskId);
    }
    return requestResponsesSingle(config, prompt, inputImageDataUrls, params, clientTaskId);
}

export async function requestGeneration(config: AiConfig & { seedIndex?: number; seedCount?: number }, prompt: string, clientTaskId?: string) {
    try {
        const images = await requestImages(config, prompt, [], clientTaskId);
        refreshRemoteUser(config);
        // 图片任务成功 → 刷新供应商账户（含 powerHistory / balanceText，LibTV 适配器在 pollTask 成功后会写记录）
        void useVendorStore.getState().loadAccounts(true);
        return images;
    } catch (error) {
...
export async function requestEdit(config: AiConfig & { seedIndex?: number; seedCount?: number }, prompt: string, references: ReferenceImage[], clientTaskId?: string) {
    try {
        const images = await requestImages(config, prompt, references, clientTaskId);
        refreshRemoteUser(config);
        // 图片任务成功 → 刷新供应商账户（含 powerHistory / balanceText，LibTV 适配器在 pollTask 成功后会写记录）
        void useVendorStore.getState().loadAccounts(true);
        return images;
...
async function createCanvasImageTaskRequest(config: AiConfig & { seedIndex?: number; seedCount?: number }, prompt: string, references: ReferenceImage[], params: ImageRequestParams, options: CanvasImageTaskOptions): Promise<RequestInit> {
    ...
    const taskChannelHeader: Record<string, string> = activeChannelKind(config) === "remote" && taskChannelId ? { "X-Model-Channel-ID": taskChannelId } : {};
    ...
    const inputImageDataUrls = references.length ? await resolveReferenceDataUrls(references) : [];
    // 供应商模式：图片走 /images/generations 简单端点（后端 vendor dispatch 接管），不走 Responses API
    ...
    // Agnes 图片模型不支持 Responses API，使用 /images/generations 端点
    if (isAgnesImageModel(config.model)) {
        const body: Record<string, unknown> = {
            model: config.model,
            prompt: withPromptGuard(config, withSystemPrompt(config, prompt)),
    ...
    // OpenAI 标准图片模型（gpt-image-2、dall-e-3）与 Gemini 图片模型（nano-banana / gemini-2-5 / gemini-3 等）
    // 走 /images/generations：避免在 OpenAI 兼容代理（如 rolldek）上 /v1/responses 404，也避免 Gemini 因
    // Responses API 不带 prompt 而报 400 "contents with text parts or prompt is required"。
    if (isOpenAIImageGenerationModel(config.model) || isGeminiImageModel(config.model)) {
        const body: Record<string, unknown> = {
            model: config.model,
            prompt: withPromptGuard(config, withSystemPrompt(config, prompt)),
        };
...
export async function fetchImageModels(config: AiConfig) {
    if (activeChannelKind(config) === "remote") return config.models;
    const channel = localChannelForActiveModel(config);
...
// 识别 OpenAI 标准图片生成模型（gpt-image-2、dall-e-3 等）。这些模型在 OpenAI 兼容代理
// （如 rolldek）里只支持 /images/generations 端点，/v1/responses 会 404 Not Found。
// 因此即便 channel 是 official 也走简单端点，不走 Responses API。
function isOpenAIImageGenerationModel(model: string) {
    const m = model.toLowerCase().replace(/[\s_]+/g, "-");
    return m.startsWith("gpt-image") || m.startsWith("dall-e") || m.startsWith("dalle-");
}

// 识别走 /images/generations 简单端点（带 prompt）的 Gemini / APIMart 图片模型。
// 后端 handler/apimart_image.go 对 nano-banana / gemini-2-5 / gemini-3 / gemini-3-1 / imagen 等
// 按 /images/generations + prompt 归一化并转给 Gemini；若走 Responses API（input+tools，无 prompt）
// 会被转发成无文本 part 的 Gemini contents，导致 400 "contents with text parts or prompt is required"。
// 因此即便 official/直连模式也要走简单端点。归一化口径与后端 normalizeAPIMartModelName 保持一致。
function isGeminiImageModel(model: string) {
    const m = model.toLowerCase().replace(/[\s_./]+/g, "-");
    return (
        m.includes("nano-banana") ||
        m.includes("gemini-2-5") ||
        m.includes("gemini-3-1") ||
        m.includes("gemini-31") ||
        m.includes("gemini-3-pro") ||
        m.includes("gemini-3-1-flash-lite") ||
        m.includes("imagen")
    );
}
...
export async function listCanvasImageTasks(config: AiConfig, sources: Array<"image-workbench" | "workflow" | "canvas" | "novel"> = []) {
