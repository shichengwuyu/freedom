...
export async function checkLocalAssetsExist(): Promise<boolean> {
    const imageStore = localforage.createInstance({ name: "freedom", storeName: "image_files" });
    const mediaStore = localforage.createInstance({ name: "freedom", storeName: "media_files" });
...
export async function migrateLocalAssetsToCloud(
    ...
    if (assets.length > 0) {
        try {
            ...
            // Save locally
            await localforage.createInstance({ name: "freedom", storeName: "app_state" })
                .setItem("freedom:asset_store", JSON.stringify({ state: finalAssets }));
    ...
    const imageCategoryStore = localforage.createInstance({ name: "freedom", storeName: "image_generation_categories" });
    const localLogs: any[] = [];
    await imageLogStore.iterate((value) => {
        localLogs.push(value);
    });
    const localCategories = (await imageCategoryStore.getItem<any[]>("freedom:image_generation_categories")) || [];
    ...
    if (localLogs.length > 0 || localCategories.length > 0) {
        try {
            ...
            await imageCategoryStore.setItem("freedom:image_generation_categories", nextLogsData.categories);
