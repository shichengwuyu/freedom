...
import { pollCanvasImageTaskStatus } from "@/services/api/image";
import { VIDEO_POLL_INTERVAL_MS } from "@/services/api/video";
import { pollCanvasAudioTaskStatus } from "@/services/api/audio";

// 普通模式最大步数
const MAX_AGENT_STEPS = 12;
// 全自动模式最大步数：支持完整剧本到视频的全流程
const MAX_AUTOPILOT_STEPS = 100;
...
export async function runCanvasAgent(input: RunCanvasAgentInput): Promise<RunCanvasAgentResult> {
    ...
    // 根据 autoPilot 模式决定最大步数
    const isAutoPilot = state.autoPilot === "full" || state.autoPilot === "checkpoint";
    const maxSteps = isAutoPilot ? MAX_AUTOPILOT_STEPS : MAX_AGENT_STEPS;
    ...
    for (let step = 0; step < maxSteps; step++) {
        ...
        if (!actions.length) {
            ...
            // 全自动模式：如果 Agent 没有返回动作但完成了当前阶段，自动推进到下一阶段
            if (isAutoPilot && state.phase !== "complete") {
                ...
                if (nextPhase && nextPhase !== state.phase) {
                    ...
                    protocolMessages = trimProtocolMessages([
                        ...
                        { role: "user" as const, content: `继续执行下一阶段：${nextPhase}` },
                    ]);
                    continue;
                }
            }
...
// 阶段推进顺序：intake → concept → script → breakdown → references → storyboard → video → audio → review → complete
function getNextPhase(currentPhase: CanvasAgentPhase): CanvasAgentPhase | null {
    const phaseOrder: CanvasAgentPhase[] = ["intake", "concept", "script", "breakdown", "references", "storyboard", "video", "audio", "review", "complete"];
    const currentIndex = phaseOrder.indexOf(currentPhase);
    if (currentIndex === -1 || currentIndex >= phaseOrder.length - 1) return null;
    return phaseOrder[currentIndex + 1];
}

// 等待异步任务完成：轮询图片/视频/音频任务状态
async function waitForAsyncTasks(
    ...
    while (pending.size > 0) {
        throwIfAborted(signal);
        onEvent?.({ status: "running", label: `等待 ${pending.size} 个任务完成...` });
        
        const results = await Promise.allSettled(
            Array.from(pending).map(async (taskId) => {
                // 根据 taskId 前缀判断任务类型
                if (taskId.startsWith("img_")) {
                    const task = await pollCanvasImageTaskStatus(taskId);
                    return { taskId, status: task.status, ok: task.status === "completed" };
                } else if (taskId.startsWith("audio_")) {
                    const task = await pollCanvasAudioTaskStatus(taskId);
                    return { taskId, status: task.status, ok: task.status === "completed" };
                }
                // 视频任务需要 config + VideoResponse 才能轮询，此处无上下文，交由调用方单独处理
                return { taskId, status: "unknown", ok: false };
            })
        );
        
        for (const result of results) {
            if (result.status === "fulfilled") {
                const { taskId, status, ok } = result.value;
