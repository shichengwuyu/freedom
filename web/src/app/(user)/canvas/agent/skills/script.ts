// ⚠ 与 /novel 工作台共享分镜剧本提示词：
// `web/src/lib/prompts/storyboard.ts` 维护同一份 user content 构造（资产参考文档 + 章节拼接）；
// handler/handler/storyboard_task.go 的 executeStoryboardTask 嵌入了语义一致的 Go 副本。
// 修改本 Skill 任何与"小说 → 单条分镜视频描述词"相关的输出协议时，务必同步以上两处。
