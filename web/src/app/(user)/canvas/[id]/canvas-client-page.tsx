...
import { ChevronLeft, ChevronRight, Copy, Globe2, Home, ImageIcon, Images, Layers3, List, Menu, Bot, Music2, PanelLeftClose, PanelLeftOpen, Plus, Redo2, Settings2, Trash2, Undo2, Upload, Video } from "lucide-react";
...
import { activeChannelKind, defaultConfig, type AiConfig, useConfigStore, useEffectiveConfig } from "@/stores/use-config-store";
...
import { modelKey, resolveVideoModelCapabilities, supportsVideoAudioGeneration, supportsVideoFrameReferences } from "@/lib/video-model-capabilities";
...
import { CanvasAgentPet } from "../components/canvas-agent-pet";
import { CanvasAgentSettings } from "../components/canvas-agent-settings";
...
import { DEFAULT_CANVAS_AGENT_PANEL, DEFAULT_CANVAS_SIDE_PANEL, flushCanvasPersistence, useCanvasStore } from "../stores/use-canvas-store";
...
1. 只输出提示词正文，不要解释。
2. 必须使用中文撰写，不要使用英文。
3. 覆盖主体、构图、风格、光线、色彩、材质、镜头和氛围。
4. 尽量写成可直接用于生图模型的完整提示词。`;
...
function InfiniteCanvasPage({ projectId }: { projectId: string }) {
    ...
    const [textDetailNodeId, setTextDetailNodeId] = useState<string | null>(null);
    ...
    // 桌宠设置从 localStorage 水合（persist skipHydration，客户端补一次）
    useEffect(() => {
        void usePetSettings.persist.rehydrate();
    }, []);

    // 节点状态变化 → 桌宠播报（节点完成提醒 / 生成中陪伴）
    const prevNodeStatusRef = useRef<Map<string, string>>(new Map());
    useEffect(() => {
        const pet = usePetSettings.getState();
        ...
        for (const node of nodes) {
            ...
            next.set(node.id, status);
            const old = prev.get(node.id);
            if (!old || old === status) continue;
            const title = node.title || node.type || "节点";
            if (status === "success" && pet.broadcastNodes) {
                message = `✅ 「${title}」完成啦~`;
            } else if (status === "error" && pet.broadcastNodes) {
                message = `⚠️ 「${title}」生成失败了`;
            } else if (status === "loading") {
                if (pet.broadcastTasks) {
                    message = `🛠 开始执行「${title}」任务`;
                } else if (pet.accompany) {
                    message = `✨ 正在生成「${title}」，我陪着~`;
                }
            }
        }
        prevNodeStatusRef.current = next;
        if (message) pet.setExternalMessage(message);
    }, [nodes]);
    ...
    const agentEffectiveConfig = useMemo(
        () => ({ ...effectiveConfig, textModel: resolvedAgentConfig.textModel || effectiveConfig.textModel, quality: resolvedAgentConfig.imageQuality, size: resolvedAgentConfig.imageSize, vquality: resolvedAgentConfig.videoQuality, videoSize: resolvedAgentConfig.videoSize, count: "1", canvasImageCount: "1" }),
    ...
    // 退出/切换页面时强制落盘：避免最近一次节点变更还停留在 400ms 防抖里就被页面卸载丢弃。
    // visibilitychange（页面隐藏，早于卸载）+ pagehide 双触发，最后卸载 cleanup 再兜底一次。
    useEffect(() => {
        const handleFlush = () => {
    ...
    // 对话数据始终保存，不受 historyPausedRef 影响
    useEffect(() => {
        if (!projectLoaded) return;
        updateProject(projectId, { chatSessions, activeChatId, agentConfig });
    }, [activeChatId, agentConfig, chatSessions, projectId, projectLoaded, updateProject]);
        ...
        const pollCanvasTasks = () => {
            ...
            videoTargets.forEach((node) => {
                ...
                void pollVideoGenerationTaskStatus(generationConfig, canvasVideoTaskFromMetadata(node.metadata))
                    ...
                    .catch((err) => {
                        // 轮询失败：标记节点为错误状态，避免永久 loading
                        console.warn("video task poll failed for node", node.id, err);
                        setNodes((prev) => prev.map((n) => n.id === node.id ? { ...n, metadata: { ...n.metadata, status: NODE_STATUS_ERROR, error: "任务状态查询失败" } } : n));
                    })
                    .finally(() => {
            ...
            imageTargets.forEach((node) => {
                ...
                void pollCanvasImageTaskStatus(node.metadata.imageTaskId)
                    ...
                    .catch((err) => {
                        console.warn("image task poll failed for node", node.id, err);
                        setNodes((prev) => prev.map((n) => n.id === node.id ? { ...n, metadata: { ...n.metadata, status: NODE_STATUS_ERROR, error: "任务状态查询失败" } } : n));
                    })
                    .finally(() => {
            ...
            audioTargets.forEach((node) => {
                ...
                void pollCanvasAudioTaskStatus(node.metadata.audioTaskId)
                    ...
                    .catch((err) => {
                        console.warn("audio task poll failed for node", node.id, err);
                        setNodes((prev) => prev.map((n) => n.id === node.id ? { ...n, metadata: { ...n.metadata, status: NODE_STATUS_ERROR, error: "任务状态查询失败" } } : n));
                    })
                    .finally(() => {
    ...
    const textDetailNode = textDetailNodeId ? nodeById.get(textDetailNodeId) || null : null;
    ...
    const deleteNodes = useCallback(
        (ids: Set<string>) => {
            ...
            setTextDetailNodeId((current) => (current && allIds.has(current) ? null : current));
    ...
    const createAndOpenProject = useCallback(() => {
        const id = createProject(`Freedom ${useCanvasStore.getState().projects.length + 1}`);
    ...
    const handleGlobalPointerMove = useCallback(
        (event: PointerEvent) => {
            // ✅ 节点拖拽优先走 PointerEvent 路径，兼容触屏/触摸板/鼠标
            if (dragRef.current.isDraggingNode) {
                const currentViewport = viewportRef.current;
                const dx = (event.clientX - dragRef.current.startX) / currentViewport.k;
                const dy = (event.clientY - dragRef.current.startY) / currentViewport.k;
    ...
    const downloadSelectedNodes = useCallback(() => {
        const selectedNodes = nodes.filter((n) => selectedNodeIds.has(n.id));
        const downloadable = selectedNodes.filter(
            (n) =>
                (isCanvasImageNodeType(n.type) || n.type === CanvasNodeType.Video || n.type === CanvasNodeType.Audio) &&
                n.metadata?.content,
        );
        if (!downloadable.length) {
            message.info("选中的节点中没有可下载的媒体文件");
            return;
        }
        downloadable.forEach((node, index) => {
        ...
        message.success(`正在下载 ${downloadable.length} 个文件`);
    ...
    const cropImageNode = useCallback(async (node: CanvasNodeData, crop: CanvasImageCropRect) => {
        ...
        try {
            ...
            void message.error(err instanceof Error ? err.message : "裁剪失败，可能是图片跨域限制导致");
            setCropNodeId(null);
        }
    }, [message]);
    ...
    const maskEditImageNode = useCallback(
        async (node: CanvasNodeData, payload: CanvasImageMaskEditPayload) => {
            ...
            if (!isAiConfigReady(generationConfig, generationConfig.model)) {
                if (activeChannelKind(generationConfig) === "remote") {
        ...
        async (nodeId: string, mode: CanvasNodeGenerationMode, prompt: string) => {
            ...
            try {
                ...
                if (mode === "video") {
                    ...
                    // 当前视频模型的参考/音频能力：优先取后台按模型配置，未配置则回退白名单推断
                    const videoCapabilities = resolveVideoModelCapabilities(videoGenerationConfig.model, useConfigStore.getState().publicSettings?.modelChannel.modelCosts?.find((item) => item.model === videoGenerationConfig.model));
                    ...
                    // 视频/音频参考仅在当前模型支持对应能力时才采纳
                    const effectiveVideoReferences = videoCapabilities.refVideo ? generationContext.referenceVideos : [];
                    const effectiveAudioReferences = videoCapabilities.refAudio ? generationContext.referenceAudios : [];
                    ...
                    const created = await createVideoGenerationTask(videoGenerationConfig, requestPrompt, { references: videoReferenceImages, firstFrame, lastFrame, videoReferences: effectiveVideoReferences, audioReferences: effectiveAudioReferences }, undefined, { clientTaskId, source: "canvas", sourceId: videoId });
        ...
        async (node: CanvasNodeData) => {
            ...
            try {
                ...
                if (node.type === CanvasNodeType.Video) {
                    ...
                    // 视频/音频参考仅在当前模型支持对应能力时才采纳
                    const effectiveVideoReferences = videoCapabilities.refVideo ? context?.referenceVideos || [] : [];
                    const effectiveAudioReferences = videoCapabilities.refAudio ? context?.referenceAudios || [] : [];
                    const created = await createVideoGenerationTask(videoGenerationConfig, requestPrompt, { references, firstFrame, lastFrame, videoReferences: effectiveVideoReferences, audioReferences: effectiveAudioReferences }, undefined, { clientTaskId: retryVideoTaskId, source: "canvas", sourceId: node.id });
        ...
        <main className="flex h-full min-h-0 overflow-hidden" style={{ background: theme.canvas.background, color: theme.node.text }}>
            ...
            <section className="relative min-w-0 flex-1 overflow-hidden">
                <CanvasTopBar
                    ...
                    onExpandAssistant={() => setPetSettingsOpen(true)}
                />
                ...
                >
                    ...
                    {visibleNodes.map((node) => (
                        <CanvasNode
                            ...
                            onViewText={(node) => setTextDetailNodeId(node.id)}
                ...
                <CanvasToolbar
                    ...
                    onDownloadSelected={downloadSelectedNodes}
                ...
                {/* 文本/剧本节点详情：只读全文预览，可滚动、可复制（图片节点走 FullscreenPreview，文本节点走这里） */}
                <Modal
                    title={textDetailNode?.title || "剧本详情"}
                    open={Boolean(textDetailNode)}
                    centered
                    width={760}
                    onCancel={() => setTextDetailNodeId(null)}
                    footer={
                        <>
                            <Button
                                icon={<Copy className="size-3.5" />}
                                onClick={async () => {
                                    const text = textDetailNode?.metadata?.content || "";
                                    if (!text) return;
                                    try {
                            ...
                            <Button type="primary" onClick={() => setTextDetailNodeId(null)}>
                                关闭
                            </Button>
                        </>
                    }
                >
                    <div className="thin-scrollbar max-h-[60vh] overflow-y-auto whitespace-pre-wrap break-words rounded-md bg-black/5 p-4 text-sm leading-6">
                        {textDetailNode?.metadata?.content || "（空）"}
                    </div>
                </Modal>
            ...
            {assistantMounted ? (
                <CanvasAssistantPanel
                    ...
                    collapsed={!agentPanel.open}
                    onCollapseStart={() =>
                        setAgentPanel((current) => ({ ...current, open: false }))
                    }
                    onCollapse={() => setAgentPanel((current) => ({ ...current, open: false }))}
