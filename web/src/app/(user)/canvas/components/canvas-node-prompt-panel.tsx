...
import { BalanceSymbol, formatBalanceYuan, requestCostCents } from "@/constant/balance";
...
export function CanvasNodePromptPanel({ node, isRunning, onPromptChange, onConfigChange, onGenerate, mentionReferences = [], videoFrameOptions = [], videoResourceOptions = [], onImageSettingsOpenChange }: CanvasNodePromptPanelProps) {
    ...
    const cents = requestCostCents({ channelMode: activeChannelKind(config), modelCosts, model: config.model, count: mode === "image" ? config.count : 1 });
