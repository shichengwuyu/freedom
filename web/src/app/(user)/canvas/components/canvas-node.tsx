...
import { ChevronRight, Image as ImageIcon, Maximize2, Music2, RefreshCw, Star, Video } from "lucide-react";
...
type CanvasNodeProps = {
    ...
    onViewText?: (node: CanvasNodeData) => void;
        ...
        >
            ...
            <div
                ...
                style={{
                    ...
                    // ✅ 统一使用 Pointer Events 防止浏览器默认手势/滚动打断节点拖动
                    touchAction: "none",
                }}
                // ✅ 用 PointerEvent 开始节点拖拽，并 setPointerCapture 确保拖动过程中事件不丢失
                onPointerDown={(event) => {
                    // 仅响应主键（鼠标左键/触摸），忽略右键
                    if (event.button !== 0 && event.pointerType === "mouse") {
                        // 非左键直接交给 onContextMenu 等其他处理器
                    }
                    try {
                        (event.currentTarget as HTMLElement).setPointerCapture(event.pointerId);
                    } catch {
                        // 部分浏览器 setPointerCapture 失败也不阻塞基础事件
                    }
...
function TextContent({ node, theme, isEditingContent, textareaRef, mentionReferences, onContentChange, onStopEditing, onGenerateImage, onViewText }: NodeContentRendererProps) {
