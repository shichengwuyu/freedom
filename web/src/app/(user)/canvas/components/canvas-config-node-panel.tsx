...
import { activeChannelKind, defaultConfig, useConfigStore, useEffectiveConfig, type AiConfig } from "@/stores/use-config-store";
import { requestCostCents, BalanceSymbol, formatBalanceYuan } from "@/constant/balance";
import { canvasThemes } from "@/lib/canvas-theme";
import { useThemeStore } from "@/stores/use-theme-store";
import { useUserStore } from "@/stores/use-user-store";
import { estimateVendorCost, type VendorType } from "@/services/api/vendor";
...
export function CanvasConfigNodePanel({ node, isRunning, inputSummary, videoFrameOptions = [], videoResourceOptions = [], onConfigChange, onGenerate, onComposerToggle }: CanvasConfigNodePanelProps) {
    ...
    const fallbackCostCents = requestCostCents({ channelMode: activeChannelKind(config), modelCosts, model: config.model, count: mode === "image" ? count : 1 });
    const [estimate, setEstimate] = useState<{ credits: number; source: "estimate" | "fallback" }>({ credits: 0, source: "fallback" });
    const activeVendorType = config.activeVendorType;
    const isVendorMode = activeVendorType !== "official" && activeVendorType;

    useEffect(() => {
        // 非供应商模式时直接沿用本地静态估算
        if (!isVendorMode) {
            setEstimate({ credits: fallbackCostCents, source: "fallback" });
            return;
        }
        const token = useUserStore.getState().token;
        if (!token) {
            setEstimate({ credits: 0, source: "fallback" });
            return;
        }
        const controller = new AbortController();
        const timer = setTimeout(() => {
            estimateVendorCost({
                vendorType: activeVendorType as VendorType,
                capability: mode,
                model: config.model,
                quality: upDreamEstimateQuality(config.quality, config.size),
                size: config.size,
                count: mode === "image" ? count : 1,
                refImageCount: inputSummary.imageCount,
                refVideoCount: inputSummary.videoCount,
                hasSound: mode === "video" && config.videoGenerateAudio === "true",
            })
                .then((res) => {
    ...
    }, [activeVendorType, mode, config.model, config.quality, config.size, config.videoGenerateAudio, count, inputSummary.imageCount, inputSummary.videoCount, fallbackCostCents, isVendorMode]);

    // 供应商模式：estimate 返回的是积分（credits），显示「N 积分」
    // 官方模式：fallbackCostCents 是金额（分），显示「¥X.XX」
    const displayCostCents = isVendorMode
        ? (estimate.source === "estimate" ? estimate.credits : 0)
        : fallbackCostCents;
    ...
    return (
        <div className="flex h-full w-full cursor-move flex-col px-3 pb-3 pt-7 text-sm" style={{ color: theme.node.text }} onWheel={(event) => event.stopPropagation()}>
            <div className="mb-2 flex items-center justify-between gap-3">
                ...
                <div className="cursor-default" onPointerDown={(event) => event.stopPropagation()} onMouseDown={(event) => event.stopPropagation()}>
                    <Segmented
                        size="small"
            ...
            <div className="mb-2 flex flex-wrap gap-1.5">
                ...
                <button type="button" className="inline-flex h-7 cursor-pointer items-center gap-1 rounded-md border px-2 text-[11px]" style={chipStyle} onPointerDown={(event) => event.stopPropagation()} onMouseDown={(event) => event.stopPropagation()} onClick={onComposerToggle}>
                    <Settings2 className="size-3.5" />
                    组装提示词
                </button>
            </div>

            <div className={`mb-2 grid min-w-0 cursor-default items-center gap-2 ${mode === "image" || mode === "video" ? "grid-cols-[minmax(0,1fr)_148px_92px]" : mode === "audio" ? "grid-cols-[minmax(0,1fr)_148px]" : "grid-cols-1"}`} onPointerDown={(event) => event.stopPropagation()} onMouseDown={(event) => event.stopPropagation()}>
            ...
            >
                <span className="inline-flex items-center gap-1.5">
                    {/* 供应商模式：显示积分；官方模式：显示金额 */}
                    {isVendorMode && estimate.source === "estimate" && displayCostCents > 0 && (
                        <>
                            <BalanceSymbol className="text-sm leading-none" />
                            <span className="text-sm font-medium">{Math.round(displayCostCents)} 积分</span>
                        </>
                    )}
                    {!isVendorMode && displayCostCents > 0 && (
                        <>
                            <BalanceSymbol className="text-sm leading-none" />
                            <span className="text-sm font-medium">{formatBalanceYuan(displayCostCents)}</span>
                        </>
                    )}
...
// 把前端 quality/size 映射成 UpDream /api/estimate 认的 quality（1K/2K/4K）
function upDreamEstimateQuality(quality: string, size: string): string {
    const lowerSize = size.toLowerCase();
    if (lowerSize.includes("4k") || lowerSize.includes("3840") || lowerSize.includes("2160")) return "4K";
    if (lowerSize.includes("2k") || lowerSize.includes("2048") || lowerSize.includes("1152")) return "2K";
