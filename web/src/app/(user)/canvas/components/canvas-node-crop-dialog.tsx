...
export function CanvasNodeCropDialog({ dataUrl, open, onClose, onConfirm }: { dataUrl: string; open: boolean; onClose: () => void; onConfirm: (crop: CanvasImageCropRect) => Promise<void> | void }) {
    ...
    const handleConfirm = async () => {
        ...
        try {
            ...
            // cropImageNode 自己已经处理了 error toast 和关闭弹窗，
            // 这里兜底防止 loading 卡死。
...
function resizeCrop(crop: CanvasImageCropRect, dx: number, dy: number, handle: ResizeHandle, aspectRatio: number | null, box: DOMRect): CanvasImageCropRect {
    ...
    if (aspectRatio !== null) {
        // box 的显示比例 = box.width / box.height，可能与 aspectRatio 不同。
        // 在归一化坐标空间中：crop.width * box.width / (crop.height * box.height) = aspectRatio
        // 所以 crop.height = crop.width * box.width / (aspectRatio * box.height)
        const heightFromWidth = (w: number) => (w * box.width) / (aspectRatio * box.height);
        const widthFromHeight = (h: number) => (h * box.height * aspectRatio) / box.width;

        if (handle === "n" || handle === "s") {
            // 纯上下拖拽：以 height 为基准推 width，水平居中
            next.width = widthFromHeight(next.height);
            next.x = crop.x + (crop.width - next.width) / 2;
        } else if (handle === "e" || handle === "w") {
            // 纯左右拖拽：以 width 为基准推 height，垂直居中
            next.height = heightFromWidth(next.width);
            next.y = crop.y + (crop.height - next.height) / 2;
        } else {
            // 角点拖拽：以 width 为基准推 height，固定对角
            next.height = heightFromWidth(next.width);
            if (handle.includes("n")) {
                next.y = crop.y + crop.height - next.height;
            }
            if (handle.includes("w")) {
                next.x = crop.x + crop.width - next.width;
            }
        }
    }
