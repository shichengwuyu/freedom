...
export async function hydrateNodeGenerationContext(context: NodeGenerationContext) {
    ...
    // 单张参考图加载失败跳过（不 abort 整批），首/尾帧失败降级为 null
    const referenceResults = await Promise.allSettled(context.referenceImages.map(async (image) => ({ ...image, dataUrl: await imageToDataUrl(image) })));
    const referenceImages = referenceResults
        .filter((result): result is PromiseFulfilledResult<ReferenceImage> => result.status === "fulfilled")
        .map((result) => result.value);
    const firstFrame = context.firstFrame ? await imageToDataUrl(context.firstFrame).then((dataUrl) => ({ ...context.firstFrame!, dataUrl })).catch(() => null) : null;
    const lastFrame = context.lastFrame ? await imageToDataUrl(context.lastFrame).then((dataUrl) => ({ ...context.lastFrame!, dataUrl })).catch(() => null) : null;
    return {
        ...context,
        referenceImages,
        firstFrame,
