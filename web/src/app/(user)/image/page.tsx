...
import { ImageSettingsPanel, imageFormatLabel, imageQualityLabel, imageQualityOptionsForModel, imageSizeLabel, imageSizeOptionsForModel } from "@/components/image-settings-panel";
import { normalizeImageQualityForModel, normalizeImageSizeForModel } from "@/lib/image-model-capabilities";
...
import { activeChannelKind, selectableModelChannels, useConfigStore, useEffectiveConfig, defaultConfig, type AiConfig } from "@/stores/use-config-store";
...
import { deleteStoredImages, getProxyUrl, imageToDataUrl, resolveImageUrl, uploadImage, uploadRemoteImageToServer } from "@/services/image-storage";
import { useAssetStore } from "@/stores/use-asset-store";
import { useWorkbenchStore } from "@/stores/use-workbench-store";
import { useUserStore } from "@/stores/use-user-store";
import { useVendorStore, type BoundAccount, type VendorType } from "@/stores/use-vendor-store";
...
type RequestSnapshot = { text: string; apiText?: string; requestConfig: AiConfig; displayConfig: GenerationLogConfig; references: ReferenceImage[] };
...
const LOG_STORE_KEY = "freedom:image_generation_logs";
const CATEGORY_STORE_KEY = "freedom:image_generation_categories";
const WORKBENCH_LAYOUT_KEY = "freedom:image-workbench-layout";
const RESULT_VIEW_MODE_KEY = "freedom:image-result-view-mode";
const IMAGE_TASK_POLL_INTERVAL_MS = 10000;
const WORKFLOW_BUTTON_POSITION_KEY = "freedom:workflow-button-position";
const logStore = localforage.createInstance({ name: "freedom", storeName: "image_generation_logs" });
const categoryStore = localforage.createInstance({ name: "freedom", storeName: "image_generation_categories" });
export default function ImagePage() {
    ...
    // 激活的供应商类型：vendor（UpDream/NewWow/LibTV）走供应商自己的计费；official 走平台余额
    ...
    // prompt / references 迁到持久化工作台 store，切换到视频创作台再返回时不会丢失
    const prompt = useWorkbenchStore((state) => state.image.prompt);
    const setPrompt = useWorkbenchStore((state) => state.setImagePrompt);
    const references = useWorkbenchStore((state) => state.image.references);
    const setReferences = useWorkbenchStore((state) => state.setImageReferences);
    ...
    const [workflowButtonPosition, setWorkflowButtonPosition] = useState(defaultWorkflowButtonPosition);
    ...
    // vendor 模式（UpDream/NewWow/LibTV）：平台 modelCosts 表无 cost，扣多少由供应商服务端定；
    // 用 powerHistory 显示「上次消耗 N」给用户参考（与 ModelPicker 下拉里的「上次 N power」标签一致）。
    ...
    const mentionCandidates = useMemo<MentionCandidate[]>(() => {
        return references.map((ref, index) => ({
            ...
            title: ref.name || imageReferenceLabel(index),
    ...
    // 最近生成图（@ 分组弹层「最近生成」区，最多 4 张成功图）
    const recentGenerated = useMemo<MentionRecentItem[]>(() => {
        const out: MentionRecentItem[] = [];
        const seen = new Set<string>();
        for (const log of logs) {
            for (const image of log.images || []) {
                if (!image.dataUrl || seen.has(image.id)) continue;
                seen.add(image.id);
                out.push({ id: image.id, title: log.title || image.id, previewUrl: image.dataUrl, storageKey: image.storageKey, mimeType: image.mimeType });
                if (out.length >= 4) return out;
            }
        }
        for (const result of results) {
            if (result.status !== "success" || !result.image?.dataUrl || seen.has(result.image.id)) continue;
            seen.add(result.image.id);
            out.push({ id: result.image.id, title: result.prompt.slice(0, 18) || result.image.id, previewUrl: result.image.dataUrl, storageKey: result.image.storageKey, mimeType: result.image.mimeType });
            if (out.length >= 4) return out;
        }
        return out;
    }, [logs, results]);

    // 素材库分组（人物/场景/物品/风格/其它），候选序号从当前参考图数量往后排
    const assetGroups = useMemo<MentionAssetGroup[]>(() => {
        let counter = references.length;
        return groupAssetsByCategory(assets).map((group) => ({
            name: group.name,
            items: group.items.map((asset) => {
                const candidate: MentionCandidate = {
                    id: asset.id,
                    label: imageReferenceLabel(counter),
                    title: asset.title,
    ...
    // 素材 / 最近生成图选中：落库为参考图并返回真实标签；不支持或失败时返回 null 取消插入
    const handleSelectAssetCandidate = (candidate: MentionCandidate): string | null | undefined => {
        ...
        if (asset) {
            if (asset.kind !== "image") {
                message.warning("视频/音频素材不能作为生图参考图");
                return null;
            }
            const dataUrl = asset.data.dataUrl || asset.coverUrl;
            if (!dataUrl) return null;
            reference = { id: nanoid(), name: asset.title, type: asset.data.mimeType || "image/png", dataUrl, storageKey: asset.data.storageKey };
        } else if (candidate.previewUrl) {
            reference = { id: nanoid(), name: candidate.title || "recent.png", type: candidate.mimeType || "image/png", dataUrl: candidate.previewUrl, storageKey: candidate.storageKey };
        }
    ...
    // 刷新后恢复参考图的有效 URL（blob: URL 刷新后会失效，需要通过 storageKey 重新获取）
    useEffect(() => {
        if (!references.length) return;
        let needsUpdate = false;
        const resolvedPromises = references.map(async (ref) => {
            if (!ref.storageKey) return ref;
            // 如果 dataUrl 为空或是 blob: URL（刷新后会失效），需要重新解析
            if (!ref.dataUrl || ref.dataUrl.startsWith("blob:")) {
                ...
                try {
                    ...
                    console.error("Failed to resolve reference image URL:", ref.id, e);
                    return ref;
                }
            }
            return ref;
        ...
        if (needsUpdate) {
            void Promise.all(resolvedPromises).then((resolved) => {
                // 按实际字段值比较，避免「resolveImageUrl 返回的是同一个缓存 blob URL、却因为对象引用不同」被误判为变化，导致无限 setReferences 闪烁
            const hasDiff = resolved.some((r, i) => r?.dataUrl !== references[i]?.dataUrl || r?.storageKey !== references[i]?.storageKey);
                if (hasDiff) {
    ...
    const usesBackendImageTasks = (value: AiConfig) => activeChannelKind(value) === "remote" || (activeChannelKind(value) === "local" && Boolean(token));
        ...
        try {
            const nextReferences = await Promise.all(
                imageFiles.map(async (file) => {
                    ...
                    // 不用上传返回的 blob: URL（session-scoped，刷新即失效），统一走 resolveImageUrl
                    // 让 storageKey 作为唯一真相源：localforage blob 还在 → 复用 cached object URL；丢了 → 重新生成
                    const dataUrl = await resolveImageUrl(image.storageKey, image.url);
                    return { id: nanoid(), name: file.name, type: image.mimeType, dataUrl: dataUrl || image.url, storageKey: image.storageKey, source: "upload" as const, temporary: true };
                }),
            );
            ...
            try {
                const nextReferences = await Promise.all(
                    blobs.map(async (blob, index) => {
                        ...
                        // 同 addReferences：统一用 resolveImageUrl 拿稳定 URL，避免 blob URL 失效闪烁
                        const dataUrl = await resolveImageUrl(image.storageKey, image.url);
                        return { id: nanoid(), name: `clipboard-${index + 1}.png`, type: image.mimeType, dataUrl: dataUrl || image.url, storageKey: image.storageKey, source: "clipboard" as const, temporary: true };
                    }),
                );
                setReferences((value) => [...value, ...nextReferences]);
    ...
    const createPersistentImageTask = async (pendingLog: GenerationLog, snapshot: RequestSnapshot, index: number, taskCount: number) => {
        try {
            const task = await createCanvasImageTask(
                ...
                snapshot.apiText || snapshot.text,
                snapshot.references,
    ...
    const addResultToReferences = async (image: GeneratedImage, index: number) => {
        try {
            if (image.storageKey) {
                ...
                // uploadImage 返回的是 blob: URL（session-scoped），统一走 resolveImageUrl
                const dataUrl = await resolveImageUrl(stored.storageKey, stored.url);
                setReferences((value) => [...value, { id: nanoid(), name: `result-${index + 1}.png`, type: stored.mimeType, dataUrl: dataUrl || stored.url, storageKey: stored.storageKey, source: "result", temporary: false }]);
            }
    ...
    const insertPickedAsset = async (payload: InsertAssetPayload) => {
        if (payload.kind === "text") {
            ...
            if (reference) {
                ...
                // uploadImage 返回 blob: URL（session-scoped），统一走 resolveImageUrl 拿稳定 URL
                const dataUrl = await resolveImageUrl(stored.storageKey, stored.url);
                setReferences((value) => [...value, { id: nanoid(), name: payload.title, type: stored.mimeType, dataUrl: dataUrl || stored.url, storageKey: stored.storageKey, source: payload.source === "library" ? "library" : "upload", temporary: payload.source !== "library" }]);
            }
        } else {
    ...
    const previewGenerationLog = async (log: GenerationLog) => {
        ...
        const normalized = normalizeImageConfigForModel(nextModel, log.config.size || "", log.config.quality || "");
        if (log.config.quality) updateConfig("quality", normalized.quality);
        if (log.config.size) updateConfig("size", normalized.size);
    ...
    const buildRequestSnapshot = ({ promptText = prompt, referenceItems = references, taskCount = generationCount, configOverride }: { promptText?: string; referenceItems?: ReferenceImage[]; taskCount?: number; configOverride?: Partial<GenerationLogConfig> } = {}) => {
        ...
        if (!isAiConfigReady(baseConfig, requestModel)) {
            if (activeChannelKind({ ...baseConfig, model: requestModel }) === "remote") {
        ...
        const normalized = normalizeImageConfigForModel(requestModel, baseConfig.size, baseConfig.quality);
        const requestConfig = { ...baseConfig, model: requestModel, imageModel: requestModel, activeChannelId: requestChannelId, imageChannelId: requestChannelId, count: "1", size: normalized.size, quality: normalized.quality };
        // 检查提示词中是否包含参考图标签（图片1、图片2 等），如果是则添加参考编号上下文帮助模型理解
        const referenceLabels = referenceItems.map((_, i) => imageReferenceLabel(i));
        const hasReferenceInPrompt = referenceLabels.some((label) => text.includes(label));
        const apiText = hasReferenceInPrompt ? buildImageReferencePromptText(text, referenceItems) : text;
        return {
            text,
            apiText,
            requestConfig,
    ...
    const runGenerationTask = async (resultId: string, snapshot: RequestSnapshot) => {
        ...
        try {
            // resultId 用作 clientTaskId（2026-08-17 改造）：透传到后端 readRequestID →
            // ConsumeUserBalanceWithHold 的 requestID，让"同一 resultId 重试"在数据库层幂等，
            // 避免前端重复点击或网络重试造成双扣。
            const result = snapshot.references.length
                ? await requestEdit(snapshot.requestConfig, snapshot.apiText || snapshot.text, snapshot.references, resultId)
                : await requestGeneration(snapshot.requestConfig, snapshot.apiText || snapshot.text, resultId);
                ...
                {workbenchLayout === "side" ? (
                    <>
                        <WorkbenchPanel
                            ...
                            activeVendorType={activeVendorType}
                            activeVendorAccount={activeVendorAccount}
...
function WorkbenchPanel({
    ...
    activeVendorAccount: BoundAccount | null;
    lastCostCents: string | number | undefined;
}) {
    const [bottomSettingsCollapsed, setBottomSettingsCollapsed] = useState(true);
    // 按模型裁剪尺寸 / 质量可选项
    const availableSizeOptions = useMemo(() => imageSizeOptionsForModel(model), [model]);
    const availableQualityOptions = useMemo(() => imageQualityOptionsForModel(model), [model]);
                    ...
                    <div className="flex flex-col gap-3">
                        <div className="grid gap-2 lg:grid-cols-[minmax(0,1fr)_auto] lg:items-start">
                            ...
                            <div className="flex shrink-0 flex-wrap items-center justify-end gap-2">
                                ...
                                <Button type="primary" className="h-9 rounded-xl lg:!hidden font-medium px-4" disabled={!canGenerate} onClick={onGenerate}>
                                    {pendingCount ? (
                                        `${pendingCount} 生成中`
                                    ) : (
                                        <span className="flex items-center gap-1">
                                            {costInfo.costCents > 0 ? (
                                                <span className="text-xs opacity-75">¥{((costInfo.costCents || 0) / 100).toFixed(2)} /张</span>
                        ...
                        <div className={`grid grid-cols-2 gap-2 sm:grid-cols-3 lg:grid-cols-[1.3fr_1.1fr_0.9fr_0.9fr_0.9fr_0.85fr_0.8fr_0.8fr_auto_auto] ${bottomSettingsCollapsed ? "hidden lg:grid" : "grid"}`}>
                            <label className="grid gap-1 text-xs text-stone-500 dark:text-stone-400">
                                ...
                                <ModelPicker
                                    ...
                                    onChange={(value, channelId) => {
                                        ...
                                        const normalized = normalizeImageConfigForModel(value, config.size, config.quality);
                                        if (normalized.size !== config.size) updateConfig("size", normalized.size);
                                        if (normalized.quality !== config.quality) updateConfig("quality", normalized.quality);
                                    }}
                                    className="canvas-compact-control !h-11 !rounded-xl !w-full"
                                    onMissingConfig={() => openConfigDialog(false)}
                                    fullWidth
                                />
                            </label>
                            <QuickSelect label="尺寸" value={config.size || "auto"} options={availableSizeOptions} onChange={(value) => updateConfig("size", value)} />
                            <QuickSelect label="质量" value={config.quality || "auto"} options={availableQualityOptions} onChange={(value) => updateConfig("quality", value)} />
                            ...
                            <Button type="primary" className="h-11 min-w-28 rounded-xl hidden lg:inline-flex" disabled={!canGenerate} onClick={onGenerate}>
                ...
                <section className="overflow-hidden rounded-lg border border-stone-200 bg-background dark:border-stone-800">
                    ...
                    <div className="border-t border-stone-200 p-3 dark:border-stone-800 space-y-2">
                        ...
                        <MentionTextarea
                            ...
                            onChange={onPromptChange}
                            rows={6}
                            placeholder="描述画面主体、风格、构图、光线和用途，可输入 @ 来指定参考图"
                            promptItems={promptItems}
                            recentGenerated={recentGenerated}
                            assetGroups={assetGroups}
                            onSelectAsset={onSelectAsset}
            ...
            <div className="shrink-0 border-t border-stone-200 p-4 dark:border-stone-800">
                <Button type="primary" size="large" block disabled={!canGenerate} onClick={onGenerate}>
                    <span className="flex items-center justify-center gap-1.5">
                        {pendingCount ? (
                            `继续提交（${pendingCount} 个生成中）`
                        ) : (
                            <>
                                {costInfo.costCents > 0 ? (
                                    <span className="text-xs opacity-75">¥{((costInfo.costCents || 0) / 100).toFixed(2)} /张</span>
                                ) : activeVendorType !== "official" && activeVendorAccount ? (
                                    // 激活的是 vendor（UpDream/NewWow/LibTV）：平台 Credits 表无 cost（vendor 自己计费），
                                    // 显式提示「按 {vendor} 金额扣减」避免误以为免费
                                    <span className="text-xs opacity-55">按 {activeVendorAccount.displayName || activeVendorType} 金额扣减{lastCostCents ? ` · 上次 ${lastCostCents}` : ""}</span>
                                ) : (
                                    <span className="text-xs opacity-55">免费</span>
                                )}
                                <span>开始生成</span>
...
function ReferenceStrip({ references, compact = false, className = "", onRemoveReference, uploadingCount = 0 }: { references: ReferenceImage[]; compact?: boolean; className?: string; onRemoveReference: (id: string) => void; uploadingCount?: number }) {
    // 独立预览 Modal：用原生 <img> + 自己的 onClick 触发，避免 antd <Image> 的 mask 拦截删除按钮的 click 事件
    // （删除按钮之前的 z-20 在 ant-image 内部 stacking context 里被 mask 的 hover 透明度遮罩 + pointer 行为干扰）
    const [previewIndex, setPreviewIndex] = useState<number | null>(null);
    const previewItem = previewIndex !== null ? references[previewIndex] ?? null : null;
    const previewSrc = previewItem ? imgSrc(previewItem.dataUrl) : undefined;
    return (
        <>
            <div
                    ...
                    <div key={item.id} className={`${compact ? "size-12" : "size-20"} group relative shrink-0 overflow-hidden rounded-md border border-stone-200 dark:border-stone-800`}>
                        ...
                        >
                            <img
                                ...
                                draggable={false}
                            />
                            <span className="pointer-events-none absolute inset-0 flex items-center justify-center bg-black/0 text-xs text-white opacity-0 transition-opacity group-hover:bg-black/40 group-hover:opacity-100">
                                点击预览
                            </span>
                        </button>
                        <button
                            type="button"
                            className="absolute right-1 top-1 z-30 flex size-6 items-center justify-center rounded bg-black/55 text-white transition-colors hover:bg-black/80"
                            onClick={(e) => { e.stopPropagation(); onRemoveReference(item.id); }}
            ...
            <Modal
                ...
                styles={{ body: { padding: 0, background: "rgb(var(--background))" } }}
            >
                {previewSrc ? (
                    <img
                        src={previewSrc}
...
function ReferenceQuickActions({ references, onUploadReferences }: { references: ReferenceImage[]; onUploadReferences: () => void }) {
    return (
        <div className="flex h-11 items-center gap-1 rounded-xl border border-stone-200 bg-background px-2 dark:border-stone-800">
            {references[0] ? <img src={imgSrc(references[0].dataUrl)} alt={references[0].name} className="size-7 rounded object-cover" /> : null}
    ...
    return (
        <div className="space-y-3">
            <section className="overflow-hidden rounded-lg border border-stone-200 bg-background dark:border-stone-800">
                ...
                <div className="border-t border-stone-200 p-3 dark:border-stone-800 space-y-2">
                    <ModelPicker config={config} value={model} capability="image" channelId={config.imageChannelId} onChange={(value, channelId) => {
                        updateConfig("imageModel", value);
                        if (channelId) updateConfig("imageChannelId", channelId);
                        const normalized = normalizeImageConfigForModel(value, config.size, config.quality);
                        if (normalized.size !== config.size) updateConfig("size", normalized.size);
                        if (normalized.quality !== config.quality) updateConfig("quality", normalized.quality);
                    }} fullWidth onMissingConfig={() => openConfigDialog(false)} />
                </div>
            </section>
    ...
    return (
        <div className="overflow-hidden rounded-lg border border-stone-200 bg-background dark:border-stone-800">
            <div className="relative aspect-[4/3] bg-stone-100 dark:bg-stone-900">
                ...
                <Image src={imgSrc(image.dataUrl)} alt={`生成结果 ${index + 1}`} className="aspect-[4/3] object-cover" />
            </div>
    ...
    return (
        <div className={`overflow-hidden rounded-lg border bg-background dark:bg-stone-950 ${active ? "border-stone-900 dark:border-stone-100" : "border-stone-200 dark:border-stone-800"}`}>
            <div className="relative aspect-[4/3] bg-stone-100 dark:bg-stone-900">
                ...
                {firstImage ? (
                    <Image src={imgSrc(firstImage.dataUrl)} alt={`历史结果 ${index + 1}`} className="aspect-[4/3] object-cover" />
                ) : (
                ...
                {displayImages.length > 1 ? (
                    <div className="absolute bottom-1.5 left-1.5 right-1.5 flex gap-1 overflow-hidden">
                        {displayImages.slice(0, 4).map((image) => (
                            <img key={image.id} src={imgSrc(image.dataUrl)} alt="" className="size-8 shrink-0 rounded border border-white/80 object-cover shadow-sm dark:border-stone-900/80" />
                        ))}
                    </div>
                ) : null}
    ...
    return (
        <div className={`absolute z-10 flex items-center gap-1 rounded-md bg-black/55 p-1 shadow-sm backdrop-blur ${className}`}>
            {visibleReferences.map((item) => (
                <img key={item.id} src={imgSrc(item.dataUrl)} alt={item.name} className="size-7 rounded border border-white/60 object-cover" />
            ))}
...
function mergeBackendImageTasks(logs: GenerationLog[], tasks: CanvasImageTask[], config: AiConfig) {
    ...
    tasks.forEach((task) => {
        ...
        const nextLog = buildLog({
            id: sourceId || task.id,
            prompt: task.prompt || "",
            model: task.model || config.imageModel || defaultConfig.imageModel,
            config: buildGenerationLogConfig({ ...config, model: task.model || config.imageModel || defaultConfig.imageModel, count: "1" }),
            references: [],
            durationMs: 0,
            successCount: 0,
            failCount: 0,
...
// 按当前图片模型把 size / quality 吸附到能力白名单，避免旧任务/切换模型后把非法值（如 Gemini 的 "21:9"）发出去。
function normalizeImageConfigForModel(model: string, size: string, quality: string): { size: string; quality: string } {
    return {
        size: normalizeImageSizeForModel(model, size || "auto"),
        quality: normalizeImageQualityForModel(model, quality || "auto"),
    };
}

// 展示层统一走后端代理：外部直链（如上游生图 CDN）在服务器/浏览器环境下可能因网络、跨域或 referer 拉不到，
// 用 /api/proxy-image 让后端代为拉取；本地 blob / data / 同源资源保持原样。
function imgSrc(url?: string) {
    return url ? getProxyUrl(url) : undefined;
}
