...
export function isSeedanceVideoModel(model: string) {
    ...
    // sd-2.x 系列视频模型（如 sd-2.5-720p、sd-2.0-fast-720p 等）和 seedance/doubao-seedance
    // 共享同一个上游（字节火山引擎），都要求 JSON 请求体，走同一套 seedance 参数格式
    return value.includes("seedance") || value.includes("doubao-seedance") || /^sd-2\./.test(value);
}
