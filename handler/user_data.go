...
func SaveUserModelConfig(w http.ResponseWriter, r *http.Request) {
	...
	if err := json.NewDecoder(r.Body).Decode(&request); err != nil || len(request.Config) == 0 || string(request.Config) == "null" {
		Fail(w, "配置内容不能为空")
		return
	}
...
func SaveUserImageHistory(w http.ResponseWriter, r *http.Request) {
	...
	if err := json.NewDecoder(r.Body).Decode(&request); err != nil || len(request.Data) == 0 || string(request.Data) == "null" {
		Fail(w, "数据内容不能为空")
		return
	}
