...
// MyAffiliateInfo 当前用户的邀请信息（邀请码、邀请人数、累计佣金）。
func MyAffiliateInfo(w http.ResponseWriter, r *http.Request) {
	user, ok := service.UserFromContext(r.Context())
	if !ok || user.ID == "" {
		Fail(w, "请先登录")
		return
	}
	info, err := service.MyAffiliateInfo(user.ID)
	if err != nil {
		FailError(w, err)
		return
	}
...
// MyAffiliateCommissions 当前用户的邀请返佣流水（分页）。
func MyAffiliateCommissions(w http.ResponseWriter, r *http.Request) {
	user, ok := service.UserFromContext(r.Context())
	if !ok || user.ID == "" {
		Fail(w, "请先登录")
		return
	}
	items, total, err := service.ListMyAffiliateCommissions(user.ID, parseQuery(r))
	if err != nil {
		FailError(w, err)
		return
	}
	OK(w, model.AffCommissionLogList{Items: items, Total: total})
}
