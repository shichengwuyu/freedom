...
func normalizeKIEInputFields(input map[string]any, modelName string) {
	...
	normalizeKIEKlingHighResVideoSize(input, modelName)
...
// 高分辨率档位（1080p / 4K）下 Kling API 要求用 video_size（字面像素），不能再发 aspect_ratio。
// mode 字段（motion-control）和 resolution 字段（v3-turbo）两种来源都要认。
// kling-3.0/video 由 normalizeKIEKlingV3VideoInput 自己处理，不在此函数重复。
func normalizeKIEKlingHighResVideoSize(input map[string]any, modelName string) {
	switch strings.ToLower(strings.TrimSpace(modelName)) {
	case "kling-2.6/motion-control", "kling-3.0/motion-control",
	...
	modeOrResolution := ""
	if v, ok := input["mode"]; ok && !isEmptyValue(v) {
		modeOrResolution = toStringSafe(v)
	} else if v, ok := input["resolution"]; ok && !isEmptyValue(v) {
		modeOrResolution = toStringSafe(v)
	}
	normalized := strings.ToLower(strings.TrimSpace(modeOrResolution))
	if normalized != "1080p" && normalized != "4k" {
		return
	}

	ratio, _ := input["aspect_ratio"].(string)
	if ratio == "" {
		return
	}
	pixelSize := klingV3VideoPixelSizeForMode(ratio, normalized)
	if pixelSize == "" {
		return
	}
	input["video_size"] = pixelSize
	delete(input, "aspect_ratio")
}
...
func normalizeKIEKlingV3VideoInput(input map[string]any, modelName string) {
	...
	// pro / 4K 模式下 Kling API 要求用 video_size（字面像素），aspect_ratio 会被拒
	if mode := strings.ToLower(strings.TrimSpace(toStringSafe(input["mode"]))); mode == "pro" || mode == "4k" {
		if ratio := strings.TrimSpace(toStringSafe(input["aspect_ratio"])); ratio != "" {
			if pixelSize := klingV3VideoPixelSizeForMode(ratio, mode); pixelSize != "" {
				input["video_size"] = pixelSize
				delete(input, "aspect_ratio")
			}
		}
...
func klingV3VideoPixelSizeForMode(aspectRatio, mode string) string {
	normalized := strings.ToLower(strings.TrimSpace(mode))
	is1080p := normalized == "pro" || normalized == "1080p"
	is4K := normalized == "4k"
	if !is1080p && !is4K {
		return ""
	}
...
func kieModelInputConfig(modelName string) kieInputConfig {
	...
	configs := map[string]kieInputConfig{
		...
		"kling-2.6/image-to-video":            {aspectField: "aspect_ratio", durationKind: "string", imageRefField: "image_urls", imageRefKind: "array"},
		"kling-2.6/text-to-video":             {aspectField: "aspect_ratio", durationKind: "string"},
		"kling-2.6/motion-control":            {aspectField: "aspect_ratio", durationKind: "string", imageRefField: "input_urls", imageRefKind: "array", videoRefField: "video_urls", videoRefKind: "array"},
		"kling-3.0/motion-control":            {aspectField: "aspect_ratio", durationKind: "string", imageRefField: "input_urls", imageRefKind: "array", videoRefField: "video_urls", videoRefKind: "array"},
		...
		"kling/v3-turbo-image-to-video":       {aspectField: "aspect_ratio", durationKind: "string", hasResolution: true, imageRefField: "image_urls", imageRefKind: "array"},
