...
type directAIUpload struct {
	...
	// AuthMode 鉴权方式：空值=渠道 apiKey（默认，上传到厂商图床）；
	// "user_token"=用户登录 token（上传到系统通用文件接口 /api/v1/files，走对象存储 provider 返回公网 URL）。
...
func directAIUploads(provider string, channel model.ModelChannel, kinds map[string]bool) (map[string]directAIUpload, error) {
	...
	case "apimart":
		// apimart 不托管音视频上传，本地音视频改为上传到系统通用文件接口 /api/v1/files，
		// 走用户配置的对象存储 provider 返回公网 URL，再传给 apimart（需用户已配置对象存储）。
		if kinds["video"] {
			uploads["video"] = directAIUpload{
				URL:           "/api/v1/files",
				FileField:     "file",
				ResponsePaths: []string{"data.url"},
				AuthMode:      "user_token",
			}
		}
		if kinds["audio"] {
			uploads["audio"] = directAIUpload{
				URL:           "/api/v1/files",
				FileField:     "file",
				ResponsePaths: []string{"data.url"},
				AuthMode:      "user_token",
