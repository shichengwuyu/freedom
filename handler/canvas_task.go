...
func CreateCanvasImageTask(w http.ResponseWriter, r *http.Request) {
	...
	// 供应商模式：用户激活了第三方供应商账户时，图片生成走适配器（用户自购额度），
	// 不需要官方 ModelChannel；runCanvasImageTask → proxyAIRequest 里的 vendor dispatch 会接管。
	...
	var resolvedUserChannelID string
	if isVendorMode {
