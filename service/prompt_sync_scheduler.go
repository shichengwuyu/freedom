...
func SyncRemotePromptCategories() {
	// 防止上一轮同步尚未结束时 cron 再次触发导致重复执行。
	promptSyncMu.Lock()
	if promptSyncRunning {
		promptSyncMu.Unlock()
		log.Printf("scheduled prompt sync skipped: previous run still in progress")
		return
	}
	promptSyncRunning = true
	promptSyncMu.Unlock()
	defer func() {
		promptSyncMu.Lock()
		promptSyncRunning = false
		promptSyncMu.Unlock()
	}()
