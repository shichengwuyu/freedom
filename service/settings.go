...
// ModelCost 取单个模型的扣费配置。
//
// 找不到价格时返回 error（而不是返回 CostCents:0）——这是防"0 元白嫖"的关键防线。
// 之前这里会静默返回 CostCents=0，导致模型列表（availableModels）里有但价格表（modelCosts）里
// 还没被自动价/手工价填入的模型能以 0 元被调用。前端下拉（model-picker.tsx）已经按 modelCosts
// 隐藏了未配价的付费云端模型，但绕过前端直接调 API 仍然能 0 元跑；这里就是后端兜底。
// 调用方应根据 error 拒绝（handler/ai.go 与 handler/video_task.go）。
func ModelCost(modelName string) (model.ModelCost, error) {
	...
	if modelName == "" {
		return model.ModelCost{}, errors.New("缺少模型名称")
	}
	...
	return model.ModelCost{}, fmt.Errorf("模型 %q 暂未配置价格", modelName)
}
