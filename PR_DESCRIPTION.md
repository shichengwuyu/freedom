...
> 📋 这是 PR 描述。复制粘贴到 https://github.com/shichengwuyu/freedom/pull/new/feature/novel-workflow
...
| `08123f7` | **Sprint 3**：公开定价 API（`/api/pricing`）+ UserGroup 用户组 + 阶梯定价 |
| `0974f50` | **Sprint 4**：通用 task 模型（`tasks` 表）+ 通用 worker（5s 轮询）+ `RegisterTaskHandler` 接口 |
| `9bf6bd5` | **Sprint 4.2**：task worker example 模板 + 方向调整文档（不做 quota / 不做会员订阅） |

**新增数据表**：`user_tokens` / `user_groups` / `tasks` / `channel_fail_logs` / `user_vendor_accounts` 等

**新增 API**：
- `POST /api/v1/user-tokens` / `GET /api/v1/user-tokens` / `DELETE /api/v1/user-tokens/:id` —— 用户 API Key
- `GET /api/pricing` —— 公开定价（不需登录）
- `GET /api/v1/tasks` —— 通用 task 列表
- `GET /api/admin/channels-health` / `POST /api/admin/channels-health/clear-cooldowns` —— 渠道健康度
- `POST /api/v1/vendor/*` —— 多 vendor 接入（UpDream / NewWow / LibTV）
...
- `openspec/changes/add-novel-video-assembly/proposal.md` + `proposal-v2.md`
- `openspec/changes/add-novel-video-assembly/design.md` + `tasks.md`
- 8 个 spec 文件在 `openspec/changes/add-novel-video-assembly/specs/`
