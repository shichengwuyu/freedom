...
func TestDefaultVendorAuthMode(t *testing.T) {
	cases := []struct {
		...
		{model.VendorTypeUpDream, model.VendorAuthModeCookie},
...
func TestNormalizeVendorAuthMode_CookieKeptForCookieVendor(t *testing.T) {
	// UpDream 默认就是 cookie：DB 里 cookie 是正确的，不应被误改成 custom_header。
	v := &model.Vendor{Type: model.VendorTypeUpDream, AuthMode: "cookie"}
	got := normalizeVendorAuthMode(model.VendorTypeUpDream, v)
	if got.AuthMode != model.VendorAuthModeCookie {
		t.Errorf("UpDream cookie mode should be kept; got %q", got.AuthMode)
	}
	if got.AuthHeaderName != "" {
		t.Errorf("UpDream AuthHeaderName should stay empty; got %q", got.AuthHeaderName)
	}
}
