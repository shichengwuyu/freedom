	...
	for key, headers := range files {
		...
		for _, header := range headers {
			...
			if strings.HasPrefix(strings.ToLower(contentType), "video/") || strings.HasPrefix(strings.ToLower(contentType), "audio/") {
				// apimart 不托管音视频上传：本地渠道直连（direct-ai）路径已支持上传到系统对象存储换公网 URL，
				// 此处为服务端代理路径，暂未实现自动转存，提示用户改用本地渠道直连或提供公网媒体地址。
				return errors.New("APIMart 渠道不支持直接上传本地音视频，请改用本地渠道直连或提供公网媒体地址")
			}
