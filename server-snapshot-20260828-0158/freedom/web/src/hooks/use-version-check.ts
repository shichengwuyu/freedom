...
const latestVersionUrl = "https://raw.githubusercontent.com/tigerowo/freedom/main/VERSION";
const latestChangelogUrl = "https://raw.githubusercontent.com/tigerowo/freedom/main/CHANGELOG.md";
...
export function useVersionCheck() {
    ...
    const checkLatestVersion = useCallback(async () => {
        try {
            ...
            const response = await fetch(latestVersionUrl, { signal: controller.signal });
            clearTimeout(timer);
        ...
        async (showMessage = false) => {
            ...
            try {
                ...
                const [versionResponse, changelogResponse] = await Promise.all([
                    fetch(latestVersionUrl, { signal: controller.signal }),
                    fetch(latestChangelogUrl, { signal: controller.signal }),
                ]);
                clearTimeout(timer);
