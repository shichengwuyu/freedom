...
import { adjustAdminUserBalance, deleteAdminUser, fetchAdminUsers, saveAdminUser, type AdminUser } from "@/services/api/admin";
...
export function useAdminUsers() {
    ...
    const balanceMutation = useMutation({
        mutationFn: ({ id, balanceCents }: { id: string; balanceCents: number }) => adjustAdminUserBalance(token, id, balanceCents),
        onSuccess: async () => {
            await queryClient.invalidateQueries({ queryKey: ["admin", "users"] });
    ...
    return {
        ...
        pageSize,
        total: data?.total || 0,
        isLoading: query.isFetching || saveMutation.isPending || deleteMutation.isPending || balanceMutation.isPending,
        ...
        adjustBalance: (id: string, balanceCents: number) => balanceMutation.mutateAsync({ id, balanceCents }),
