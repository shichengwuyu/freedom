...
import { CheckCircleOutlined, CopyOutlined, DeleteOutlined, EditOutlined, ExportOutlined, EyeOutlined, PlusOutlined, ReloadOutlined, SearchOutlined, SyncOutlined, CloseCircleOutlined } from "@ant-design/icons";
import { ProTable, type ProColumns } from "@ant-design/pro-components";
import { Button, Card, Col, Flex, Form, Image, Input, Modal, Row, Select, Segmented, Space, Table, Tag, Tooltip, Typography } from "antd";
...
export default function AdminPromptsPage() {
    ...
    const statusLabel = (status: string) => status === "pending" ? "待审核" : status === "rejected" ? "已拒绝" : "已通过";
    const statusColor = (status: string) => status === "pending" ? "orange" : status === "rejected" ? "red" : "green";
    ...
    const columns: ProColumns<Prompt>[] = [
        ...(activeTab !== "all" ? [{
            ...
            dataIndex: "status",
            width: 80,
            render: (_: unknown, item: Prompt) => <Tag color={statusColor(item.status)}>{statusLabel(item.status)}</Tag>,
        }] : []),
        {
            title: "封面",
            dataIndex: "coverUrl",
            ...
            render: (_, item) => (
                <Space size={4}>
                    ...
                    {activeTab === "pending" ? (
                        <>
                            <Tooltip title="通过">
                                <Button type="text" size="small" icon={<CheckCircleOutlined />} style={{ color: "#52c41a" }} onClick={() => void approvePrompt(item.id)} />
                            </Tooltip>
                            <Tooltip title="拒绝">
                                <Button type="text" size="small" icon={<CloseCircleOutlined />} style={{ color: "#ff4d4f" }} onClick={() => void rejectPrompt(item.id)} />
                            </Tooltip>
                        </>
                    ) : null}
                    {activeTab === "all" ? (
    ...
    return (
        <main style={{ padding: 24 }}>
            <Flex vertical gap={16}>
                <Segmented
                    value={activeTab}
                    onChange={(v) => setActiveTab(v as "all" | "pending" | "rejected")}
                    options={[
                        { label: "全部", value: "all" },
