...
从分镜剧本生成视频时，\`@角色名\` 标签的用途是**绑定角色身份**，同时保留动作描述：

- \`@沈一寻\`、\`@段嘉怡\` 这些标签表示”这个角色由对应图片参考锁定身份”。
- **动作描述**（如”低眉”、”抬头”、”跪靠近”、”轻握她的手”、”轻喘”等）**必须保留在视频提示词中**，视频生成需要表现这些动作。
- 图片生成只引用角色资产（静态肖像/四视图），视频生成引用角色资产+动作描述。
- 示例：分镜写”@沈一寻低眉，轻握她的手”，视频生成时提示词写”图片1中的沈一寻低眉，轻握她的手”，图片1锁定沈一寻的身份。
