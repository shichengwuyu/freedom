...
从分镜剧本生成图片时，\`@角色名\` 标签的用途是**引用角色资产图片**，不是描述动作：

- \`@沈一寻\`、\`@段嘉怡\` 这些标签只表示"使用这个角色对应的图片参考"，图片生成时只应引用角色的静态肖像/四视图。
- **动作描述**（如"低眉"、"抬头"、"跪靠近"、"轻握她的手"、"轻喘"等）**不应出现在图片提示词中**，这些动作是视频生成阶段才需要表现的。
- 图片的提示词只关注：角色身份、场景环境、构图、光线、景别；动作留给视频。
- 示例：分镜写"@沈一寻低眉，轻握她的手"，图片生成时只引用沈一寻的角色图片，提示词写"图片1中的沈一寻"，不要写"沈一寻低眉"或"沈一寻轻握她的手"。
