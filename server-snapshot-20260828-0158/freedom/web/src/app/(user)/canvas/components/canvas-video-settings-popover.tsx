...
import { normalizeVideoAspectRatioForModel, normalizeVideoResolutionForModel, normalizeVideoSecondsForModel, supportsVideoFrameReferences } from "@/lib/video-model-capabilities";
...
export function CanvasVideoSettingsPopover({ config, onConfigChange, frameOptions = [], resourceOptions = [], metadata, firstFrameNodeId, lastFrameNodeId, onFrameChange, onMetadataChange, buttonClassName, buttonIcon, placement = "topLeft", visualOnly = false }: CanvasVideoSettingsPopoverProps) {
    ...
    const vModel = config.model || config.videoModel || "";
    // 按钮简略展示与面板 snap 保持一致
    const labelResolution = videoResolutionLabel(normalizeVideoResolutionForModel(vModel, config.vquality || "720"));
    const labelSize = videoSizeLabel(normalizeVideoAspectRatioForModel(vModel, config.size || "auto"));
    const labelSeconds = visualOnly ? null : videoSecondsLabel(normalizeVideoSecondsForModel(vModel, config.videoSeconds || "6"));
            ...
            <span ref={buttonRef} className="inline-flex min-w-0">
                <Button size="small" type="text" className={buttonClassName || "!h-8 !max-w-[170px] !justify-start !rounded-full !px-2.5"} style={{ background: theme.node.fill, color: theme.node.text }} icon={buttonIcon || <Settings2 className="size-3.5" />} onClick={() => setOpen((current) => !current)}>
                    <span className="truncate">
                        {labelResolution} · {labelSize}
                        {labelSeconds ? <> · {labelSeconds}</> : null}
                    </span>
                </Button>
            </span>
            {panel}
    ...
    return (
        <>
            ...
            {!isKIEKlingV3 ? <CanvasSettingGroup title="负面提示词" color={theme.node.muted}>
                <Input.TextArea value={config.videoNegativePrompt || ""} placeholder="描述不希望出现在视频中的内容" autoSize={{ minRows: 3, maxRows: 6 }} className="rounded-xl placeholder:!text-[var(--canvas-placeholder)] placeholder:!opacity-55" style={{ background: theme.node.fill, borderColor: theme.node.stroke, color: theme.node.text, WebkitTextFillColor: theme.node.text, "--canvas-placeholder": theme.node.placeholder } as CSSProperties} onPointerDown={(event) => event.stopPropagation()} onMouseDown={(event) => event.stopPropagation()} onChange={(event) => onConfigChange("videoNegativePrompt", event.target.value)} />
...
function OptionPill({ selected, theme, onClick, children }: { selected: boolean; theme: (typeof canvasThemes)[keyof typeof canvasThemes]; onClick: () => void; children: ReactNode }) {
    return <button type="button" className="h-9 cursor-pointer rounded-full border px-2 text-sm transition hover:opacity-80" style={{ background: "transparent", borderColor: selected ? theme.node.text : theme.node.stroke, color: theme.node.text }} onPointerDown={(event) => event.stopPropagation()} onMouseDown={(event) => event.stopPropagation()} onClick={onClick}>{children}</button>;
}
