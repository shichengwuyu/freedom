...
export function CanvasImageSettingsPopover({ config, onConfigChange, onOpenChange, buttonClassName, placement = "topLeft", showSize = true, showCount = true, buttonIcon }: CanvasImageSettingsPopoverProps) {
    ...
    // 按钮上的简略展示：与面板里的 snap 逻辑保持一致（模型切换后旧值会被自动归一）
    const quality = normalizeImageQualityForModel(config.model || "", config.quality || "auto");
    const count = Math.max(1, Math.min(15, Math.floor(Math.abs(Number(config.count)) || 1)));
    const activeSize = normalizeImageSizeForModel(config.model || "", config.size || "auto");
