...
import { AlertCircle, ArrowLeft, ArrowRight, BookOpen, CheckSquare, ChevronDown, ChevronUp, ClipboardPaste, CloudUpload, Copy, Download, FolderPlus, History, LoaderCircle, Music2, PanelBottom, PanelLeft, Plus, RotateCcw, SlidersHorizontal, Trash2, Upload, VideoIcon } from "lucide-react";
...
import { boolConfig, buildSeedancePromptText, isSeedanceVideoConfig, normalizeSeedanceRatio, seedanceReferenceLabel, seedanceVideoReferenceError, seedanceVideoReferenceHint, SEEDANCE_REFERENCE_LIMITS } from "@/lib/seedance-video";
import { filterVideoResolutionOptions, getModelMaxDuration, modelKey, resolveVideoModelCapabilities, type VideoModelCapabilities } from "@/lib/video-model-capabilities";
...
import { activeChannelKind, selectableModelChannels, useConfigStore, useEffectiveConfig, type AiConfig, type VideoElementItem, type VideoElementReference } from "@/stores/use-config-store";
import { useThemeStore } from "@/stores/use-theme-store";
import { useUserStore } from "@/stores/use-user-store";
import { useVendorStore } from "@/stores/use-vendor-store";
import { formatCostCents, requestCostCents } from "@/constant/balance";
import type { ReferenceImage } from "@/types/image";
import type { ReferenceAudio, ReferenceVideo } from "@/types/media";
import { MentionTextarea, type MentionAssetGroup, type MentionCandidate, type MentionPromptItem, type MentionRecentItem } from "@/app/(user)/components/mention-textarea";
import { groupAssetsByCategory } from "@/lib/asset-groups";
...
const WORKBENCH_LAYOUT_KEY = "freedom:video-workbench-layout";
const logStore = localforage.createInstance({ name: "freedom", storeName: "video_generation_logs" });
export default function VideoPage() {
    ...
    // 激活的供应商类型：vendor 模式走供应商自己的计费；official 走平台余额
    const activeVendorType = useConfigStore((state) => state.config.activeVendorType);
    const activeVendorAccount = useVendorStore((s) =>
        activeVendorType === "official" ? null : s.accounts.find((a) => a.vendorType === activeVendorType) ?? null,
    );
    ...
    // 工作台输入迁到持久化 store，切换到生图工作台再返回时不会丢失参考图/首尾帧/参考视频音频等
    const prompt = useWorkbenchStore((state) => state.video.prompt);
    const setPrompt = useWorkbenchStore((state) => state.setVideoPrompt);
    const negativePrompt = useWorkbenchStore((state) => state.video.negativePrompt);
    const setNegativePrompt = useWorkbenchStore((state) => state.setVideoNegativePrompt);
    const references = useWorkbenchStore((state) => state.video.references);
    const setReferences = useWorkbenchStore((state) => state.setVideoReferences);
    const firstFrame = useWorkbenchStore((state) => state.video.firstFrame);
    const setFirstFrame = useWorkbenchStore((state) => state.setVideoFirstFrame);
    const lastFrame = useWorkbenchStore((state) => state.video.lastFrame);
    const setLastFrame = useWorkbenchStore((state) => state.setVideoLastFrame);
    const videoReferences = useWorkbenchStore((state) => state.video.videoReferences);
    const setVideoReferences = useWorkbenchStore((state) => state.setVideoReferenceVideos);
    const audioReferences = useWorkbenchStore((state) => state.video.audioReferences);
    const setAudioReferences = useWorkbenchStore((state) => state.setVideoReferenceAudios);
    ...
    // vendor 模式（UpDream/NewWow/LibTV）：用 powerHistory 显示「上次消耗 N」与 ModelPicker 下拉对齐
    const lastCostCents = useMemo(() => {
        if (activeVendorType === "official") return undefined;
        return activeVendorAccount?.powerHistory?.[model]?.power;
    }, [activeVendorAccount, activeVendorType, model]);
    const generateCostCents = requestCostCents({
        channelMode: activeChannelKind({ ...effectiveConfig, model }),
        modelCosts,
        model,
        count: taskCount,
        seconds: videoConfig.videoSeconds,
    });
    const costInfo = formatCostCents({
        channelMode: activeChannelKind({ ...effectiveConfig, model }),
        modelCosts,
        model,
        count: taskCount,
    ...
    // 当前视频模型的参考/音频能力：优先取后台按模型配置，后台未配置则回退白名单推断
    const videoCapabilities = useMemo(() => resolveVideoModelCapabilities(model, modelCosts?.find((item) => item.model === model)), [model, modelCosts]);
    const pendingLogCount = logs.filter((log) => log.status === "生成中" && log.task && !log.video).length;
    // 参考素材对应的 @ 候选项（图片、视频、音频合并为同一个有序列表）
    const mentionCandidates = useMemo<MentionCandidate[]>(() => {
        ...
        references.forEach((ref, index) => {
            items.push({
                ...
                title: ref.name || seedanceReferenceLabel("image", index),
        ...
        videoReferences.forEach((ref, index) => {
            items.push({
                ...
                title: ref.name || seedanceReferenceLabel("video", index),
                previewUrl: ref.url,
                mediaUrl: ref.url,
                kind: "video",
                index,
        ...
        audioReferences.forEach((ref, index) => {
            items.push({
                id: ref.id,
                label: seedanceReferenceLabel("audio", index),
                title: ref.name || seedanceReferenceLabel("audio", index),
                mediaUrl: ref.url,
                kind: "audio",
                index,
            });
    ...
    // 预设提示词（@ 分组弹层顶部「选择预设提示词」，取前 6 条）
    const { items: promptListItems } = usePromptList({ keyword: "", tags: [], category: ALL_PROMPTS_OPTION, enabled: true });
    const promptItems = useMemo<MentionPromptItem[]>(
        () => promptListItems.slice(0, 6).map((item) => ({ id: item.id, title: item.title, coverUrl: item.coverUrl, prompt: item.prompt })),
        [promptListItems],
    );

    // 素材库分组（人物/场景/物品/风格/其它），图片/视频/音频各自按当前参考数量往后排
    const assetGroups = useMemo<MentionAssetGroup[]>(() => {
        let imageCounter = references.length;
        let videoCounter = videoReferences.length;
        let audioCounter = audioReferences.length;
        return groupAssetsByCategory(assets).map((group) => ({
            name: group.name,
            items: group.items.map((asset) => {
                let label = "";
                let index = 0;
                if (asset.kind === "image") {
                    label = seedanceReferenceLabel("image", imageCounter);
                    index = imageCounter;
                    imageCounter += 1;
                } else if (asset.kind === "video") {
                    label = seedanceReferenceLabel("video", videoCounter);
                    index = videoCounter;
                    videoCounter += 1;
                } else {
                    label = seedanceReferenceLabel("audio", audioCounter);
                    index = audioCounter;
                    audioCounter += 1;
                }
                const candidate: MentionCandidate = {
                    ...
                    label,
                    title: asset.title,
                    previewUrl: asset.kind === "image" ? asset.data.dataUrl : asset.coverUrl,
                    mediaUrl: asset.kind === "video" || asset.kind === "audio" ? asset.data.url : undefined,
                    kind: asset.kind,
                    index,
                    asset,
                };
    ...
    }, [assets, references.length, videoReferences.length, audioReferences.length]);

    // 素材 / 最近生成图选中：按类型落库为参考图/视频/音频并返回真实标签；失败返回 null 取消插入
    const handleSelectAssetCandidate = (candidate: MentionCandidate): string | null | undefined => {
        const asset = candidate.asset;
        if (asset) {
            if (asset.kind === "image") {
                const dataUrl = asset.data.dataUrl || asset.coverUrl;
                if (!dataUrl) return null;
                let index = -1;
                setReferences((value) => {
                    index = value.length;
                    return [...value, { id: nanoid(), name: asset.title, type: asset.data.mimeType || "image/png", dataUrl, storageKey: asset.data.storageKey }];
                });
                if (index < 0) return null;
                return seedanceReferenceLabel("image", index);
            }
            if (asset.kind === "video") {
                const url = asset.data.url || asset.coverUrl;
                if (!url) return null;
                let index = -1;
                setVideoReferences((value) => {
                    index = value.length;
                    return [...value, { id: nanoid(), name: asset.title, type: asset.data.mimeType || "video/mp4", url, storageKey: asset.data.storageKey, bytes: asset.data.bytes, width: asset.data.width, height: asset.data.height }];
                });
                if (index < 0) return null;
                return seedanceReferenceLabel("video", index);
            }
            if (asset.kind === "audio") {
                ...
                setAudioReferences((value) => {
                    index = value.length;
                    return [...value, { id: nanoid(), name: asset.title, type: asset.data.mimeType || "audio/mpeg", url, storageKey: asset.data.storageKey, durationMs: asset.data.durationMs }];
                });
                if (index < 0) return null;
        ...
        if (candidate.previewUrl) {
            ...
            const previewStorageKey = candidate.storageKey;
            let index = -1;
            setReferences((value) => {
                index = value.length;
                return [...value, { id: nanoid(), name: candidate.title || "recent.png", type: candidate.mimeType || "image/png", dataUrl: previewUrl, storageKey: previewStorageKey }];
            });
            if (index < 0) return null;
            return seedanceReferenceLabel("image", index);
        }
    ...
    // 刷新后恢复参考素材的有效 URL（blob: URL 刷新后会失效，需要通过 storageKey 重新获取）
    useEffect(() => {
        // 恢复图片参考
        if (references.some(ref => ref.storageKey && (!ref.dataUrl || ref.dataUrl.startsWith("blob:")))) {
            void Promise.all(references.map(async (ref) => {
                ...
                if (!ref.dataUrl || ref.dataUrl.startsWith("blob:")) {
                    try {
                        ...
                        return url ? { ...ref, dataUrl: url } : ref;
                    } catch (e) {
                        console.error("Failed to resolve image reference URL:", ref.id, e);
                        return ref;
                    }
                }
                return ref;
        ...
        // 恢复首帧
        if (firstFrame?.storageKey && (!firstFrame.dataUrl || firstFrame.dataUrl.startsWith("blob:"))) {
            void resolveImageUrl(firstFrame.storageKey, firstFrame.dataUrl || "")
                .then(url => url && setFirstFrame({ ...firstFrame, dataUrl: url }))
                .catch(e => console.error("Failed to resolve firstFrame URL:", e));
        }

        // 恢复尾帧
        if (lastFrame?.storageKey && (!lastFrame.dataUrl || lastFrame.dataUrl.startsWith("blob:"))) {
            void resolveImageUrl(lastFrame.storageKey, lastFrame.dataUrl || "")
                .then(url => url && setLastFrame({ ...lastFrame, dataUrl: url }))
                .catch(e => console.error("Failed to resolve lastFrame URL:", e));
        }

        // 恢复视频参考
        if (videoReferences.some(ref => ref.storageKey && (!ref.url || ref.url.startsWith("blob:")))) {
            void Promise.all(videoReferences.map(async (ref) => {
                if (!ref.storageKey) return ref;
                if (!ref.url || ref.url.startsWith("blob:")) {
                    try {
                        const url = await resolveMediaUrl(ref.storageKey, ref.url || "");
                        return url ? { ...ref, url } : ref;
                    } catch (e) {
                        console.error("Failed to resolve video reference URL:", ref.id, e);
                        return ref;
                    }
                }
                return ref;
            })).then(setVideoReferences);
        }

        // 恢复音频参考
        if (audioReferences.some(ref => ref.storageKey && (!ref.url || ref.url.startsWith("blob:")))) {
            void Promise.all(audioReferences.map(async (ref) => {
                if (!ref.storageKey) return ref;
                if (!ref.url || ref.url.startsWith("blob:")) {
                    try {
                        const url = await resolveMediaUrl(ref.storageKey, ref.url || "");
                        ...
                        console.error("Failed to resolve audio reference URL:", ref.id, e);
                        return ref;
                    }
                }
                return ref;
            })).then(setAudioReferences);
        }
    }, [references, firstFrame, lastFrame, videoReferences, audioReferences, setReferences, setFirstFrame, setLastFrame, setVideoReferences, setAudioReferences]);

    const usesBackendVideoTasks = (value: AiConfig) => activeChannelKind(value) === "remote" || (activeChannelKind(value) === "local" && Boolean(token));
    ...
    useEffect(() => {
        // ✅ 优化：布局读取同步，日志加载延后 120ms 让首帧先渲染完，不阻塞点击跳转的即时响应
        try {
        ...
        const logsLoadTimer = window.setTimeout(() => {
            void refreshLogs().then((items) => syncBackendVideoTasks(items));
        }, 120);
        return () => window.clearTimeout(logsLoadTimer);
    }, []);
    ...
    useEffect(() => {
        ...
        // ✅ 优化：账号历史同步延后 800ms 执行，避免与本地日志读取争抢主线程 / IndexedDB 连接
    ...
    const addReferences = async (files?: FileList | null) => {
        ...
        // Kling 工作台仅支持参考图；其他模型是否放开视频/音频参考由当前模型能力决定
        const allowVideoRef = !isKlingWorkbench && videoCapabilities.refVideo;
        const allowAudioRef = !isKlingWorkbench && videoCapabilities.refAudio;
        const unsupported = selectedFiles.filter((file) => {
            if (file.type.startsWith("image/")) return false;
            if (file.type.startsWith("video/")) return !allowVideoRef;
            if (isSupportedAudioFile(file)) return !allowAudioRef;
            return true;
        });
        if (unsupported.length) message.warning(isKlingWorkbench ? "当前 Kling 仅支持参考图" : "当前模型不支持部分参考素材，请使用图片" + (allowVideoRef ? "、mp4/mov 视频" : "") + (allowAudioRef ? "、mp3/wav 音频" : ""));
        ...
        const videoFiles = allowVideoRef ? selectedFiles.filter((file) => file.type.startsWith("video/") && file.size <= SEEDANCE_REFERENCE_LIMITS.videoMaxBytes).slice(0, SEEDANCE_REFERENCE_LIMITS.videos - videoReferences.length) : [];
        const audioFiles = allowAudioRef ? selectedFiles.filter((file) => isSupportedAudioFile(file) && file.size <= SEEDANCE_REFERENCE_LIMITS.audioMaxBytes).slice(0, SEEDANCE_REFERENCE_LIMITS.audios - audioReferences.length) : [];
    ...
    const buildRequestSnapshot = ({ promptText = prompt, negativePromptText, referenceItems = references, firstFrameItem = firstFrame, lastFrameItem = lastFrame, videoReferenceItems = videoReferences, audioReferenceItems = audioReferences, taskCountValue = taskCount, configValue = videoConfig, modelValue = model }: { promptText?: string; negativePromptText?: string; referenceItems?: ReferenceImage[]; firstFrameItem?: ReferenceImage | null; lastFrameItem?: ReferenceImage | null; videoReferenceItems?: ReferenceVideo[]; audioReferenceItems?: ReferenceAudio[]; taskCountValue?: number; configValue?: AiConfig; modelValue?: string } = {}) => {
        ...
        if (!isAiConfigReady(configValue, modelValue)) {
            if (activeChannelKind({ ...configValue, model: modelValue }) === "remote") {
                if (!modelValue.trim()) {
        ...
        const capabilities = resolveVideoModelCapabilities(modelValue, modelCosts?.find((item) => item.model === modelValue));
        const frameReferencesEnabled = !kling && capabilities.refVideo;
        // 如果提示词里含有"图片1""视频1""音频1"等标签，则添加素材编号前缀帮助模型理解
        const allLabels = mentionCandidates.map((c) => c.label);
        const hasReferenceInPrompt = allLabels.some((label) => text.includes(label));
        const effectiveRefs = [...referenceItems].slice(0, kling ? 2 : referenceItems.length);
        // 视频/音频参考仅在当前模型支持对应能力时才发送
        const effectiveVideos = kling || !capabilities.refVideo ? [] : [...videoReferenceItems];
        const effectiveAudios = kling || !capabilities.refAudio ? [] : [...audioReferenceItems];
        const apiText = hasReferenceInPrompt ? buildSeedancePromptText(text, effectiveRefs, effectiveVideos, effectiveAudios) : text;
        return { text, apiText, model: modelValue, config: buildVideoConfig({ ...configValue, videoNegativePrompt: currentNegativePrompt }, modelValue), references: effectiveRefs, firstFrame: frameReferencesEnabled ? firstFrameItem : null, lastFrame: frameReferencesEnabled ? lastFrameItem : null, videoReferences: effectiveVideos, audioReferences: effectiveAudios, taskCount: normalizeVideoCount(taskCountValue) };
    };

    const submitGenerationSnapshot = async (snapshot: { text: string; apiText?: string; model: string; config: AiConfig; references: ReferenceImage[]; firstFrame?: ReferenceImage | null; lastFrame?: ReferenceImage | null; videoReferences: ReferenceVideo[]; audioReferences: ReferenceAudio[]; taskCount: number }) => {
        setRunning(true);
        setPreviewLog(null);
        setNow(Date.now());
    ...
    const runVideoTask = async (pendingLog: GenerationLog, snapshot: { text: string; apiText?: string; model: string; config: AiConfig; references: ReferenceImage[]; firstFrame?: ReferenceImage | null; lastFrame?: ReferenceImage | null; videoReferences: ReferenceVideo[]; audioReferences: ReferenceAudio[]; taskCount: number }) => {
        try {
            const created = await createVideoGenerationTask(snapshot.config, snapshot.apiText || snapshot.text, { references: snapshot.references, firstFrame: snapshot.firstFrame, lastFrame: snapshot.lastFrame, videoReferences: snapshot.videoReferences, audioReferences: snapshot.audioReferences }, (progress) => {
    ...
    return (
        <div className="flex h-full flex-col overflow-hidden bg-stone-50 text-stone-900 dark:bg-stone-950 dark:text-stone-100">
                ...
                {workbenchLayout === "side" ? (
                    <>
                        ...
                        <WorkbenchPanel
                            ...
                            onGenerate={() => void generate()}
                            generateCostCents={generateCostCents}
                            costInfo={costInfo}
                            mentionCandidates={mentionCandidates}
                            promptItems={promptItems}
                            recentGenerated={[]}
                            assetGroups={assetGroups}
                            onSelectAsset={handleSelectAssetCandidate}
                        />
                        )}
                        <ResultsPanel
                            results={results}
            ...
            <input
                ref={fileInputRef}
                type="file"
                accept={isKlingWorkbench ? "image/*" : ["image/*", videoCapabilities.refVideo ? "video/mp4,video/quicktime" : "", videoCapabilities.refAudio ? "audio/mpeg,audio/wav,audio/x-wav,.mp3,.wav" : ""].filter(Boolean).join(",")}
                multiple
                className="hidden"
                onChange={(event) => {
...
function WorkbenchPanel({
    ...
    generateCostCents = 0,
    costInfo = { costCents: 0, unit: "", isPerSecond: false },
    mentionCandidates,
    promptItems,
    recentGenerated,
    assetGroups,
    ...
    generateCostCents?: number;
    costInfo?: { costCents: number; unit: string; isPerSecond: boolean };
    mentionCandidates: MentionCandidate[];
    promptItems: MentionPromptItem[];
    recentGenerated: MentionRecentItem[];
    assetGroups: MentionAssetGroup[];
    onSelectAsset: (candidate: MentionCandidate) => string | null | undefined;
}) {
    const frameReferencesEnabled = videoCapabilities.refVideo;
    const audioGenerationEnabled = videoCapabilities.genAudio;
    // vendor 模式（UpDream/NewWow/LibTV）：按钮上显示「按 X 积分扣减 · 上次 N 积分」
    const activeVendorType = useConfigStore((state) => state.config.activeVendorType);
    const activeVendorAccount = useVendorStore((s) =>
        activeVendorType === "official" ? null : s.accounts.find((a) => a.vendorType === activeVendorType) ?? null,
    );
                    ...
                    <div className="flex flex-col gap-3">
                        <div className="grid gap-2 lg:grid-cols-[minmax(0,1fr)_auto] lg:items-start">
                            <MentionTextarea
                                ...
                                onSubmit={() => canGenerate && onGenerate()}
                                placeholder="描述镜头运动、主体动作、场景氛围和画面风格，可输入 @ 选择参考图/视频/音频"
                                autoSize={{ minRows: 2, maxRows: 4 }}
                                promptItems={promptItems}
                                recentGenerated={recentGenerated}
                                assetGroups={assetGroups}
                                onSelectAsset={onSelectAsset}
                                className="w-full resize-none rounded-2xl border border-stone-200 bg-white px-3 py-2 text-sm text-stone-900 outline-none placeholder:text-stone-400 focus:border-sky-500 focus:ring-2 focus:ring-sky-500/20 dark:border-stone-700 dark:bg-stone-900 dark:text-stone-100 dark:placeholder:text-stone-500"
                            />
                            <div className="flex shrink-0 flex-wrap items-center justify-end gap-2">
                                ...
                                <Button type="primary" className="h-9 rounded-xl px-4 font-medium lg:!hidden" disabled={!canGenerate} onClick={onGenerate}>
                                    {pendingCount ? (
                                        `${pendingCount} 生成中`
                                    ) : (
                                        <span className="flex items-center gap-1">
                                            {costInfo.costCents > 0 ? (
                                                <span className="text-xs opacity-75">¥{((costInfo.costCents || 0) / 100).toFixed(2)} {costInfo.isPerSecond ? "/秒" : "/次"}</span>
                                            ) : activeVendorType !== "official" && activeVendorAccount ? (
                                                <span className="text-xs opacity-55">按 {activeVendorAccount.displayName || activeVendorType} 积分扣减{lastCostCents ? ` · 上次 ${lastCostCents} 积分` : ""}</span>
                                            ) : (
                                                <span className="text-xs opacity-55">免费</span>
                                            )}
                                            <span>开始创作</span>
                                        </span>
                                    )}
                        ...
                        <div className={`grid grid-cols-2 gap-2 sm:grid-cols-3 ${bottomSettingsGridClass} ${bottomSettingsCollapsed ? "hidden lg:grid" : "grid"}`}>
                            <label className="grid gap-1 text-xs text-stone-500 dark:text-stone-400">
                                模型
                                <ModelPicker config={config} value={model} channelId={config.videoChannelId} onChange={(value, channelId) => { updateConfig("videoModel", value); if (channelId) updateConfig("videoChannelId", channelId); updateConfig("videoSeconds", String(getModelMaxDuration(value))); }} capability="video" className="canvas-compact-control !h-11 !rounded-xl !w-full" onMissingConfig={() => openConfigDialog(false)} fullWidth />
                            </label>
                            {klingBottom ? (
                                ...
                                <>
                                    <QuickSelect label="清晰度" value={normalizeVideoResolutionValue(config.vquality)} options={filterVideoResolutionOptions(videoResolutionOptions, model)} onChange={(value) => { updateConfig("vquality", value); updateConfig("size", videoSizeForResolution(value, config.size)); }} />
                                    ...
                                    <QuickNumber label="秒数" value={normalizeVideoSeconds(config.videoSeconds)} min={1} max={getModelMaxDuration(model)} onChange={(value) => updateConfig("videoSeconds", value)} />
                            ...
                            <Button type="primary" className="hidden h-11 min-w-28 items-center justify-center gap-1.5 rounded-xl lg:flex" disabled={!canGenerate} onClick={onGenerate}>
    ...
    return (
        <div className="flex min-h-[420px] flex-col overflow-hidden rounded-lg border border-stone-200 bg-card shadow-sm dark:border-stone-800 lg:min-h-0">
                ...
                <WorkbenchSection title="提示词">
                    <div className="space-y-2">
                        ...
                        <MentionTextarea
                            ...
                            assetGroups={assetGroups}
                            onSelectAsset={onSelectAsset}
                            className="w-full resize-none rounded-lg border border-stone-200 bg-white px-3 py-2 text-sm text-stone-900 outline-none placeholder:text-stone-400 focus:border-sky-500 focus:ring-2 focus:ring-sky-500/20 dark:border-stone-800 dark:bg-stone-900 dark:text-stone-100 dark:placeholder:text-stone-500"
                        />
                    </div>
                </WorkbenchSection>
            ...
            <div className="shrink-0 border-t border-stone-200 p-4 dark:border-stone-800">
                <Button type="primary" size="large" block disabled={!canGenerate} onClick={onGenerate}>
                    {pendingCount ? `生成中（${pendingCount}）` : (
    ...
    return (
        <div className="space-y-3">
            <WorkbenchSection title="模型">
                <ModelPicker config={config} value={model} channelId={config.videoChannelId} onChange={(value, channelId) => { updateConfig("videoModel", value); if (channelId) updateConfig("videoChannelId", channelId); updateConfig("videoSeconds", String(getModelMaxDuration(value))); }} capability="video" fullWidth onMissingConfig={() => openConfigDialog(false)} />
            </WorkbenchSection>
...
function buildDisplayConfig(config: AiConfig, model: string): GenerationLogConfig {
    return {
        channelMode: activeChannelKind(config),
...
// 参考图顺序箭头已移除（不再需要调节左右位置，保持上传顺序即可）
function ReferenceOrderButtons(_props: { index: number; total: number; onMove: (offset: number) => void }) {
    return null;
}
...
function resolveVideoChannelId(config: AiConfig, model: string, ...preferredIds: Array<string | undefined>) {
    const channels = selectableModelChannels(config).map((channel) => ({ id: channel.id, models: channel.models }));
...
function videoChannelText(config: AiConfig, model: string) {
    ...
    const record = channel as { id?: string; name?: string; baseUrl?: string } | undefined;
    return [record?.id, record?.name, record?.baseUrl].filter(Boolean).join(" ").toLowerCase();
}
