// Next.js App Router 路由级 Loading 组件：视频创作台
...
export default function VideoLoading() {
    return (
        <div className="flex h-full w-full flex-col gap-3 bg-background p-4 text-stone-950 dark:text-stone-100 md:gap-5 md:p-6">
            ...
            <div className="grid min-h-0 flex-1 grid-cols-1 gap-3 overflow-hidden md:gap-5 xl:grid-cols-[420px_minmax(0,1fr)]">
                {/* Left: settings skeleton */}
                <div className="flex min-h-0 flex-col gap-4 overflow-y-auto rounded-xl border border-stone-200 bg-white p-4 shadow-sm dark:border-stone-800 dark:bg-stone-900 md:p-5">
                    <div className="h-28 w-full animate-pulse rounded-lg bg-stone-200 dark:bg-stone-800" />
                ...
                <div className="flex min-h-0 flex-col gap-4 overflow-hidden rounded-xl border border-stone-200 bg-white p-4 shadow-sm dark:border-stone-800 dark:bg-stone-900 md:p-5">
                    ...
                    <div className="grid flex-1 min-h-0 auto-rows-min grid-cols-1 gap-4 overflow-y-auto pr-1 md:grid-cols-2">
                        {Array.from({ length: 4 }).map((_, i) => (
                            <div
                                key={i}
                                className="aspect-video animate-pulse rounded-xl border border-stone-200 bg-stone-200 dark:border-stone-800 dark:bg-stone-800"
                            />
                        ))}
                    </div>
                </div>
