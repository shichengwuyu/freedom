...
function LoginContent() {
    ...
    const inviterCodeParam = searchParams.get("inviterCode") || "";

    useEffect(() => {
        // OAuth 回调：后端通过 cookie 设置了 auth token，前端 hydrateUser 拉取用户信息。
        // 如果 URL 中有 error 参数则显示错误；否则尝试 hydrateUser 然后跳转。
        const error = searchParams.get("error");
        if (error) {
            message.error(error);
            return;
        }
        if (searchParams.get("redirect") || window.location.hash) {
    ...
    }, [message, redirect, router, searchParams, hydrateUser]);
    ...
    const submit = async (values: LoginFormValues) => {
        try {
            ...
            if (mode === "register" && values.inviterCode) {
                Object.assign(payload, { inviterCode: values.inviterCode.trim() });
            }
    ...
    return (
        <main className="flex h-full min-h-0 items-center justify-center overflow-y-auto bg-background bg-[radial-gradient(#e5e7eb_1px,transparent_1px)] px-6 py-10 [background-size:16px_16px] dark:bg-[radial-gradient(rgba(245,245,244,.16)_1px,transparent_1px)]">
            <section className="w-full max-w-[420px]">
                ...
                <Form<LoginFormValues> layout="vertical" size="large" requiredMark={false} onFinish={submit} initialValues={{ inviterCode: inviterCodeParam }}>
                    <Form.Item>
                        <Segmented
                            block
                            value={mode}
                    ...
                    {mode === "register" ? (
                        <>
                            ...
                            <Form.Item name="inviterCode" label={<span className="font-medium text-stone-800 dark:text-stone-200">邀请码（选填）</span>}>
                                <Input placeholder="如有好友邀请码，可在此填写" autoComplete="off" />
                            </Form.Item>
                        </>
                    ) : null}
                        ...
                        {linuxDoEnabled ? (
                            <Button
                                ...
                                href={(() => {
                                    ...
                                    if (inviterCodeParam) params.set("inviterCode", inviterCodeParam);
                                    return `/api/auth/linux-do/authorize?${params.toString()}`;
                                })()}
                                icon={<img src="/icons/linuxdo.svg" alt="" width={18} height={18} />}
                            >
                                使用 Linux.do 登录
                            </Button>
                        ) : null}
