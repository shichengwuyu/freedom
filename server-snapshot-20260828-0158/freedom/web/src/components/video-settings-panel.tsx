...
import { filterVideoAspectRatioOptions, filterVideoDurationOptions, filterVideoResolutionOptions, getVideoModelOptions, modelKey, normalizeVideoAspectRatioForModel, normalizeVideoResolutionForModel, normalizeVideoSecondsForModel, supportsVideoAudioGeneration } from "@/lib/video-model-capabilities";
import { activeChannelKind, channelIdForActiveModel, localChannelForActiveModel, type AiConfig } from "@/stores/use-config-store";
...
export function VideoSettingsPanel({ config, modelName, onConfigChange, theme, showTitle = true, className = "w-[320px] space-y-4 rounded-2xl px-1 py-0.5", hideNegativePrompt = false, visualOnly = false }: VideoSettingsPanelProps) {
    ...
    const videoOptions = getVideoModelOptions(model);
    const grokMode = config.videoMode === "fun" || config.videoMode === "spicy" ? config.videoMode : "normal";
    // 按模型过滤可选项；未识别模型（null）→ 用全集默认
    const availableResolutions = filterVideoResolutionOptions(videoResolutionOptions, model);
    const availableAspectRatios = filterVideoAspectRatioOptions(seedanceRatioOptions, model);
    const availableDurations = videoOptions ? videoOptions.durations : secondOptions;
    // 当前值若不在合法集 → 自动 snap 到首项
    const resolution = videoOptions ? normalizeVideoResolutionForModel(model, normalizeVideoResolutionValue(config.vquality)) : normalizeVideoResolutionValue(config.vquality);
    const normalizedSize = normalizeVideoSizeValue(config.size);
    const size = videoOptions ? normalizeVideoAspectRatioForModel(model, normalizedSize) : normalizedSize;
    const baseSeconds = config.videoSeconds || "6";
    const seconds = videoOptions
        ? normalizeVideoSecondsForModel(model, baseSeconds)
        : secondOptions.map(String).includes(baseSeconds)
            ? baseSeconds
    ...
    const updateResolution = (value: string) => {
        const nextResolution = videoOptions ? normalizeVideoResolutionForModel(model, value) : normalizeVideoResolutionValue(value);
                    ...
                    <>
                        <SettingGroup title="秒数" color={theme.node.muted}>
                            <div className="grid grid-cols-3 gap-2.5">
                                ...
                                <NumberInput value={seconds} min={1} max={videoOptions?.maxDuration || 30} theme={theme} onChange={(value) => onConfigChange("videoSeconds", value)} />
                            </div>
                        </SettingGroup>
