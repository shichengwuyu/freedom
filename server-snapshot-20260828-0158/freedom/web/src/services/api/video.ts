...
import { activeChannelKind, buildApiUrl, channelIdForActiveModel, directAIProviderForConfig, localChannelForActiveModel, type AiConfig, type VideoElementReference } from "@/stores/use-config-store";
...
function aiHeaders(config: AiConfig) {
    ...
    if (kind === "remote") return { Authorization: `Bearer ${token}`, ...(channelIdForActiveModel(config) ? { "X-Model-Channel-ID": channelIdForActiveModel(config) } : {}) };
...
function isQueueFullError(message: string): boolean {
    return /queue is full|队列已满|please retry later/i.test(message);
}
...
export async function createVideoGenerationTask(config: AiConfig, prompt: string, references: ReferenceImage[] | VideoReferenceInput = [], onProgress?: VideoProgressHandler, options?: string | VideoTaskCreateOptions): Promise<CreatedVideoGenerationTask> {
    ...
    for (let attempt = 0; attempt <= maxRetries; attempt++) {
        try {
            ...
            if (isQueueFullError(message) && attempt < maxRetries) {
    ...
    throw new VideoRequestError("视频生成失败：服务端繁忙，请稍后重试", null);
}
...
export async function pollCreatedVideoGenerationTask(config: AiConfig, task: VideoResponse, { startedAt = Date.now(), requestBody, initialDelayMs = 0, onProgress, onPoll }: { startedAt?: number; requestBody?: unknown; initialDelayMs?: number; onProgress?: VideoProgressHandler; onPoll?: (task: VideoResponse) => void } = {}) {
    ...
    const MAX_POLL_DURATION_MS = 30 * 60 * 1000; // 30 分钟总超时
    const MAX_POLL_ATTEMPTS = 720; // 约 30 分钟（按 2.5s 间隔）
    let attemptCount = 0;
    try {
        if (initialDelayMs > 0) await new Promise((resolve) => setTimeout(resolve, initialDelayMs));
        for (; ;) {
            if (Date.now() - startedAt > MAX_POLL_DURATION_MS || attemptCount >= MAX_POLL_ATTEMPTS) {
                throw new VideoRequestError("视频生成超时（已超过 30 分钟），请稍后重试或检查任务状态", task);
            }
            attemptCount++;
        ...
        // 视频任务成功 → 刷新供应商账户（含 powerHistory / balanceText，LibTV 适配器在 pollTask 成功后会写记录）
...
async function createGrok2APIVideoRequestBody(config: AiConfig, model: string, prompt: string, input: Required<VideoReferenceInput>) {
    ...
    const urls = await resolveReferenceDataUrls(references);
...
async function createSeedanceRequestBody(config: AiConfig, model: string, prompt: string, input: Required<VideoReferenceInput>) {
    // seedance 上游要求 JSON 请求体（FormData 会被拒绝并返回 422
    // "Input should be a valid dictionary or object to extract fields from"），
    // 因此这里用普通对象，由 axios 自动以 application/json 发送。
    const body: Record<string, unknown> = {
        model,
        prompt,
        // seconds 必须为正数，否则后端会拒绝（按秒计费校验）
        seconds: normalizeVideoSecondsForModel(model, config.videoSeconds),
        // seedance 的 size 字段是比例（如 "16:9"）或 "adaptive"，不是像素尺寸
        size: normalizeSeedanceRatio(config.size),
        resolution_name: normalizeVideoResolution(config.vquality),
        preset: "normal",
    };
    // 参考图：seedance 接受 image/reference_images 数组（与 grok2api 一致）
    const references = [input.firstFrame, ...input.references, input.lastFrame].filter((reference): reference is ReferenceImage => Boolean(reference));
    const urls = await resolveReferenceDataUrls(references);
    if (urls.length) body.image = { url: urls[0] };
    if (urls.length > 1) body.reference_images = urls.slice(1).map((url) => ({ url }));
    if (supportsVideoAudioGeneration(model)) body.video_generate_audio = String(boolConfig(config.videoGenerateAudio, false));
    return body;
}
...
async function createVideoRequestBody(config: AiConfig, model: string, prompt: string, input: Required<VideoReferenceInput>) {
    ...
    // seedance 走 JSON 请求体分支，避免 FormData 触发上游 422 错误
    if (isSeedanceVideoConfig(config)) return createSeedanceRequestBody(config, model, prompt, input);
    if (isAgnesVideoModel(model)) {
        const references = input.references;
        const inputReferences = (await Promise.all(references.slice(0, 7).map(imageToAgnesReference))).filter((ref) => ref && ref.trim() !== "");
    ...
    // kling-3.0-omni 系列（如 kling-3.0-omni-720p/1080p）走 rolldek 等非 apimart/kie 渠道，
    // 不匹配 isAPIMartKlingV3/isKIEKlingV3，会错误落入 else 分支用 seconds/size/preset，
    // 导致上游 kling 服务无法识别参数而卡在 processing。这里基于模型名识别，让它走 kling v3 参数格式。
    const klingOmni = isKlingOmniVideoModel(model);
    const motionControl = apimartMotionControl || kieMotionControl;
    const klingV3 = apimartKlingV3 || kieKlingV3 || klingOmni;
...
// kling-3.0-omni 系列（如 kling-3.0-omni-720p/1080p）走非 apimart/kie 渠道（如 rolldek），
// 不匹配 isAPIMartKlingV3/isKIEKlingV3 的渠道文本判断，需要基于模型名单独识别，
// 让它走 kling v3 参数格式（mode/duration/aspect_ratio），否则会落入 else 分支用 seconds/size/preset，
// 上游 kling 服务无法识别这些参数，任务会卡在 processing。
function isKlingOmniVideoModel(model: string) {
    return modelKey(model).includes("kling-3-0-omni");
}
...
function videoChannelText(config: AiConfig, model: string) {
    ...
    const channels = [...(config.publicChannels || []), localChannelForActiveModel(scopedConfig)].filter(Boolean);
...
export function isCompletedVideoStatus(status?: string) {
