...
type DirectUploadSpec = { url: string; fileField: string; fileNameField?: string; extraFields?: Record<string, string>; responsePaths: string[]; authMode?: "user_token" };
...
export async function createDirectVideoTask(config: AiConfig, provider: DirectAIProvider, body: DirectRequestBody): Promise<DirectVideoResponse> {
    ...
    for (let attempt = 0; attempt <= maxRetries; attempt++) {
        try {
            ...
            if (/queue is full|队列已满|please retry later/i.test(message) && attempt < maxRetries) {
    ...
    throw new Error("视频生成失败：服务端繁忙，请稍后重试");
}
...
async function uploadDirectReference(spec: DirectUploadSpec, file: File, apiKey: string) {
    ...
    // user_token 模式用用户登录 token（上传到系统 /api/v1/files 走对象存储 provider 返回公网 URL）；
    // 默认模式用渠道 apiKey（上传到厂商图床如 apimart image / kie fileStream）。
    const authToken = spec.authMode === "user_token" ? useUserStore.getState().token : apiKey;
    const response = await fetch(spec.url, { method: "POST", headers: { Authorization: `Bearer ${authToken}` }, body: formData });
