...
// 全局共享的 UserConfig 请求缓存（10 秒），所有模块调用都走同一套缓存，避免重复发请求
let userConfigCache: { token: string; timestamp: number; promise: Promise<UserConfigPayload> } | null = null;
const USER_CONFIG_CACHE_MS = 10_000;

export async function fetchUserConfig(token: string, { force = false }: { force?: boolean } = {}) {
    const now = Date.now();
    if (!force && userConfigCache && userConfigCache.token === token && now - userConfigCache.timestamp < USER_CONFIG_CACHE_MS) {
        return userConfigCache.promise;
    }
    const promise = apiGet<UserConfigPayload>("/api/v1/user-config", undefined, token);
    userConfigCache = { token, timestamp: now, promise };
    return promise;
}
