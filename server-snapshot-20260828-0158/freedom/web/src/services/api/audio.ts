...
import { activeChannelKind, buildApiUrl, channelIdForActiveModel, localChannelForActiveModel, type AiConfig } from "@/stores/use-config-store";
...
function aiHeaders(config: AiConfig, clientTaskId?: string): Record<string, string> {
    const token = useUserStore.getState().token;
    // 2026-08-17 引入：可选 X-Request-Id 头把 clientTaskId 透传到后端 readRequestID,
    // 让 ConsumeUserBalanceWithHold 拿到稳定幂等键，避免前端重复提交/网络重试造成双扣。
    const headers: Record<string, string> = { "Content-Type": "application/json" };
    if (clientTaskId) headers["X-Request-Id"] = clientTaskId;
    if (activeChannelKind(config) === "remote") {
        if (token) headers["Authorization"] = `Bearer ${token}`;
        const cid = channelIdForActiveModel(config);
        if (cid) headers["X-Model-Channel-ID"] = cid;
        return headers;
    }
    if (token) {
        headers["Authorization"] = `Bearer ${token}`;
        const cid = channelIdForActiveModel(config);
        if (cid) headers["X-User-Model-Channel-ID"] = cid;
        return headers;
    }
    headers["Authorization"] = `Bearer ${localChannelForActiveModel(config)?.apiKey || config.apiKey}`;
    return headers;
}
...
export async function requestAudioGeneration(config: AiConfig, prompt: string, clientTaskId?: string, referenceAudio?: ReferenceAudio): Promise<Blob> {
    ...
    try {
        if (isMimoTtsModel(model) && !usesAccountProxy(config)) {
            ...
            const response = await axios.post<MiMoAudioResponse>(aiApiUrl(config, "/chat/completions"), body, { headers: aiHeaders(config, clientTaskId) });
        ...
        const response = await axios.post<Blob>(aiApiUrl(config, "/audio/speech"), body, { headers: aiHeaders(config, clientTaskId), responseType: "blob" });
...
export async function createCanvasAudioTask(config: AiConfig, prompt: string, options: CanvasAudioTaskOptions = {}, referenceAudio?: ReferenceAudio): Promise<CanvasAudioTask> {
    ...
    if (!usesAccountProxy(config)) {
        const blob = await requestAudioGeneration(config, prompt, options.clientTaskId, referenceAudio);
        const format = isMimoTtsModel(model) ? normalizeMimoTtsFormat(config.mimoTtsFormat) : normalizeAudioFormatValue(config.audioFormat);
    ...
    const response = await fetch("/api/v1/canvas/audio-tasks", {
        method: "POST",
        headers: aiHeaders(config, options.clientTaskId),
        body: JSON.stringify({
...
function assertAudioConfig(config: AiConfig, model: string) {
    ...
    if (activeChannelKind(config) !== "local") return;
    if (!isMimoTtsModel(model)) {
