...
import { canUseGlobalStorage, getMediaProxyUrl, loadUserStorageProvider, toProviderPayload, type StorageConfig } from "@/services/image-storage";
import { useUserStore } from "@/stores/use-user-store";

export type UploadedFile = { url: string; storageKey: string; bytes: number; mimeType: string; width?: number; height?: number; durationMs?: number };

const store = localforage.createInstance({ name: "freedom", storeName: "media_files" });
...
async function uploadMediaBlobToServer(blob: Blob, filename: string): Promise<UploadedFile> {
    ...
    // 用本地 blob 读取视频宽高，避免用服务端返回 URL 再把整个视频拉一遍
    const localMetaUrl = URL.createObjectURL(blob);
    const meta = payload.data.mimeType?.startsWith("video/") ? await readVideoMeta(localMetaUrl) : {};
